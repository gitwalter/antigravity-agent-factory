---
description: Standard Statistical Analysis Routine for Dashboard users.
dashboard: true
version: 1.0.0
tags:
- dashboard
- analysis
- routine
- standardized
---


# Statistical Analysis Routine

**Version:** 1.0.0

Follow this routine to professionally analyze a dataset using the dashboard's toolkit.

## Trigger Conditions
- A user requests a statistical analysis of a dataset.
- A new dataset is loaded and requires profiling.
- Ad-hoc exploratory analysis is needed for decision-making.

**Trigger Examples:**
- "Analyze this dataset for trends and correlations."
- "Run the standard analysis routine on the uploaded CSV."

## Steps

### 1. Data Cleaning & Inspection
- Check for NULLs or outliers in the **Explorer** view.
- Verify data types (ensure dates are parsed correctly).

### 2. Hypothesis Definition
- Based on the **📚 KPI Dictionary**, identify which metric you are optimizing (e.g., UPH or Accuracy).

### 3. Visual Discovery
- Generate a **Heatmap** to identify spatial or categorical clusters.
- Run a **Regression** to find correlations between independent variables (e.g., Temperature vs. Process Time).

### 4. Significance Testing
- Use the **🔬 Statistical Primer** to choose the right test (t-test, Z-score).
- Confirm if the findings are statistically significant.

### 5. AI Commentary
- Use the **🤖 AI Workspace** to generate a final executive summary.


## Trigger Examples
- "Execute this workflow."


## Best Practices
- **Axiomatic Alignment**: Ensure Truth, Beauty, and Love.
- **Memory First**: Check context before execution.
- **Verifiability**: Document every step.


## Related
- [workflow-standard.md](file:///.agent/rules/workflow-standard.md)
