...
type: skill
knowledge: [version-registry.json]
---

# Version Check Skill

Performs on-demand version audits for packages, blueprints, and projects using `knowledge/version-registry.json` as the authoritative source of truth for recommended versions.

## When to Use

- When checking if a package version is current
- When auditing blueprint dependency versions
- When finding outdated packages in a project
- When planning version updates with migration guidance
- When validating project dependencies against Factory standards
...
```yaml
load_registry:
  file: knowledge/version-registry.json
  structure:
    packages:
...
audit_blueprint:
  input:
    blueprint_file: "blueprints/python-fastapi/blueprint.json"
...
1. **Registry is source of truth** - Always use `knowledge/version-registry.json` as authoritative
...
| Error | Resolution |
|-------|------------|
| Registry file not found | Report error, suggest checking knowledge/ directory |
...
- `knowledge/version-registry.json` - Source of truth for package versions
- `blueprints/*/blueprint.json` - Blueprint dependency definitions
