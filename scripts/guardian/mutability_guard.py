...
class MutabilityGuard:
    ...
    def can_modify(self, path: str) -> ValidationResult:
        ...
        logger.warning(
            f"Path not in known locations, allowing with caution: {normalized}"
        )
        return ValidationResult(
            allowed=True,
    ...
    def validate_modification(
        self, path: str, content: str, check_content: bool = True
    ) -> ValidationResult:
        """
    ...
    def _check_content_for_violations(self, content: str) -> ValidationResult:
        ...
        return ValidationResult(
            allowed=True, reason="Content validation passed", policy="content_valid"
        )
