...
class ChangelogHelper:
    ...
    def check_needs_update(self) -> Tuple[bool, str]:
        ...
        return (
            False,
            f"Only {significant_count} significant files (threshold: {self.SIGNIFICANT_THRESHOLD})",
        )
...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check if changelog needs update"
    )
    parser.add_argument(
        "--suggest", action="store_true", help="Suggest changelog entries"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate changelog format"
    )
