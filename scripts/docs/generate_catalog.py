...
def generate_catalog():
    ...
    with open(catalog_path, 'w', encoding='utf-8') as f:
        ...
        for bp in blueprints:
            ...
            f.write(f"| [**{bp['name']}**]({rel_bp_path}) (`{bp['id']}`) | {bp['description']} | {tags} |\n")
        f.write("\n")
        ...
        f.write("|------|------|\n")
        for k in knowledge:
             f.write(f"| {k['name']} | [`{k['path'].replace('../../', '')}`]({k['path']}) |\n")
        f.write("\n")
        ...
        count = 0
        for t in templates:
            f.write(f"| {t['name']} | {t['category']} | [`{t['path'].replace('../../', '')}`]({t['path']}) |\n")
            count += 1
        f.write(f"\n*Total templates: {count}*\n")

    print(f"Catalog generated at docs/reference/catalog.md")
