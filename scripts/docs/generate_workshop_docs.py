...
def generate_markdown(workshop: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(
        f"**Technology:** {tech.get('language', 'N/A')} with {tech.get('stack', 'N/A')} ({tech.get('version', 'latest')})"
    )
    lines.append("")
    ...
    if objectives:
        ...
        for i, obj in enumerate(objectives, 1):
            if isinstance(obj, dict):
                lines.append(
                    f"{i}. **{obj.get('objective', 'N/A')}** ({obj.get('bloom_level', 'apply').title()})"
                )
            else:
    ...
    if exercises:
        ...
        for ex in exercises:
            ...
            if ex.get("description"):
                ...
                lines.append("")
            lines.append(
                f"**Difficulty:** {ex.get('difficulty', 'medium').title()} | "
    ...
    lines.append("")
    lines.append(
        f"**Workshop Definition:** `.agent/patterns/workshops/{workshop.get('workshopId', 'unknown')}.json`"
    )
...
def generate_index(workshops: list[dict[str, Any]]) -> str:
    ...
    lines.append("")
    lines.append(
        f"This folder contains readable documentation for all **{len(workshops)} learning workshops** in the Antigravity Agent Factory."
    )
    lines.append("")
