...
class RepoAnalyzer:
    ...
    def __init__(
        self, repo_path: Union[str, Path], factory_root: Optional[Path] = None
    ):
    ...
    def _analyze_cursorrules(self) -> AntigravityruleAnalysis:
        ...
        try:
            ...
            print(f"Warning: Could not read .agentrules: {e}")
    ...
    def _analyze_cursor_folder(self, inventory: RepoInventory) -> None:
        ...
        if agents_path.exists():
            inventory.existing_agents = [f.stem for f in agents_path.glob("*.md")]
        ...
        if commands_path.exists():
            inventory.existing_commands = [f.stem for f in commands_path.glob("*.md")]

        # Analyze rules
        rules_path = cursor_path / "rules"
        if rules_path.exists():
            inventory.existing_rules = [f.name for f in rules_path.glob("*.mdc")]
    ...
    def _analyze_project_artifacts(self, inventory: RepoInventory) -> None:
        ...
        if workflows_path.exists():
            inventory.existing_workflows = [
                f.name for f in workflows_path.glob("*") if f.is_file()
            ]
    ...
    def _determine_scenario(self, inventory: RepoInventory) -> OnboardingScenario:
        ...
        has_cursor_folder = bool(
            ...
            or inventory.existing_skills
            or inventory.existing_commands
            or inventory.existing_rules
            or inventory.mcp.exists
        )
        has_project_artifacts = bool(
            inventory.existing_knowledge
            or inventory.has_purpose_md
            or inventory.has_practices_yaml
            or inventory.has_methodology_yaml
        )
        ...
        if inventory.agentrules.has_factory_marker:
