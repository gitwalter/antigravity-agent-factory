...
class GitHubAdapter(BaseAdapter):
    ...
    async def _get_session(self) -> "aiohttp.ClientSession":
        ...
        if aiohttp is None:
            raise ImportError(
                "aiohttp is required for GitHubAdapter. Install with: pip install aiohttp"
            )
    ...
    async def fetch_updates(
        ...
        if target_files:
            repos_to_check = [
                r for r in repos_to_check if r.knowledge_file in target_files
            ]
    ...
    async def _check_releases(
        self, repo: TrackedRepository, since: Optional[datetime] = None
    ) -> List[KnowledgeUpdate]:
    ...
    def _parse_release_notes(
        self, notes: str, repo: TrackedRepository
    ...
    def _determine_priority(
        self, release: Dict[str, Any], changes: List[KnowledgeChange]
    ) -> UpdatePriority:
    ...
    def _is_breaking(self, release: Dict[str, Any]) -> bool:
        ...
        return any(
            indicator in body or indicator in tag for indicator in breaking_indicators
        )
