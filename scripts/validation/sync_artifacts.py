...
class ArtifactScanner:
    ...
    def scan(self, config: ArtifactConfig) -> list[ArtifactInfo]:
        ...
        for path in matches:
            ...
            artifacts.append(ArtifactInfo(id=artifact_id, path=path, metadata=metadata))
    ...
    def _should_exclude(
        self, path: Path, excludes: list[str], source_dir: Path
    ) -> bool:
        ...
        for exclude in excludes:
            ...
            if exclude.endswith("/"):
                ...
                if (
                    rel_path.startswith(dir_name + "/")
                    or f"/{dir_name}/" in f"/{rel_path}"
                ):
                    return True
...
class CategoryCountsSyncStrategy(SyncStrategy):
    ...
    def sync(
        ...
        actual_counts = get_actual_counts(self.root_path)

        for category, config in target.categories.items():
            # Map category name to CategoryTestCounts field
            field_name = category.lower()
            actual_count = getattr(actual_counts, field_name, 0)

            # Extract current count from file
            ...
            if abs(old_count - actual_count) > tolerance:
                ...
                new_content = re.sub(
                    pattern,
                    f"| {category.title()} Tests | ~{actual_count} |",
                    new_content,
                    flags=re.IGNORECASE,
                )
                ...
                if match:
                    ...
                    if old_count != count:
                        ...
                        new_line = re.sub(
                            ...
                            f"({count} "
                            + target.annotation_pattern.split()[0]
                            .replace("\\(", "")
                            .replace("\\d+", str(count))
                            + ")",
                            line,
...
class MarkdownTableSyncStrategy(SyncStrategy):
    ...
    def sync(
        ...
        sorted_artifacts = sorted(
            ...
            key=lambda a: (
                a.id,
                str(a.path.relative_to(self.root_path)).replace("\\", "/"),
            ),
        )
        ...
        for artifact in sorted_artifacts:
            ...
            for col in target.columns:
                if col == target.id_column:
                    ...
                    val = artifact.metadata.get(col) or artifact.metadata.get(
                        col.lower(), ""
                    )
            ...
            if in_target_section:
                ...
                if not table_replaced:
                    ...
                    if line.strip().startswith("|") and all(
                        c in line for c in target.columns[:2]
                    ):
                        new_lines.append(new_table)
                        table_replaced = True
    ...
    def sync(
        ...
        if new_content == content:
            return SyncResult(
                ...
                target_type="catalog",
                changed=False,
                old_value=len(test_cases),
                new_value=len(test_cases),
                message="Already in sync",
            )
...
class SyncEngine:
    ...
    def __init__(
        self, root_path: Optional[Path] = None, config_path: Optional[Path] = None
    ):
    ...
    def sync_artifact(
        self, artifact_name: str, dry_run: bool = True, use_fast_count: bool = False
    ) -> list[SyncResult]:
        ...
        if config.count_method == "pytest_collect":
            if use_fast_count:
                ...
                # Optimized: use the unified get_actual_counts
                count = get_actual_counts(self.root_path).total
        elif config.count_method == "file_count" and config.source_dir == "tests":
            count = self.scanner.count_test_functions()
        else:
...
def collect_test_count(
    test_dir: Optional[str] = None, root_path: Optional[Path] = None
) -> int:
...
def get_actual_counts(root_path: Optional[Path] = None) -> CategoryTestCounts:
    ...
    Performs a single optimized collection of all tests and categorizes them
    by their relative path to avoid multiple slow subprocess calls.

    Args:
        root_path: Root path of the project. Defaults to current working directory.

    Returns:
        CategoryTestCounts with counts for each test category.
    """
    if root_path is None:
        root_path = Path.cwd()

    print("Collecting test counts from pytest (optimized single pass)...")

    tests_path = root_path / "tests"
    if not tests_path.exists():
        return CategoryTestCounts(0, 0, 0, 0, 0, 0)

    try:
        # Single collection call for ALL tests
        result = subprocess.run(
            [
                sys.executable,
                "-m",
        ...
        # Parse the collection output to look for test items
        # pytests's -q output for collection looks like file paths
        ...
        # Pattern: tests/category/...::test_name
        # Note: We need to be careful with parametrized tests which appear multiple times
        # Collect unique test IDs
        test_ids = [line for line in lines if "::" in line]
        total = len(test_ids)

        if total > 0:
            print(
                f"DEBUG: Collected {total} test IDs. First 5: {test_ids[:5]}, Last 5: {test_ids[-5:]}"
            )
        ...
        print(f"Error collecting counts: {e}")
        return CategoryTestCounts(0, 0, 0, 0, 0, 0)
...
def extract_documented_counts(content: str) -> CategoryTestCounts:
    ...
    """
    # Pattern for total: "consists of **942 tests**" - flexible for approximate counts
    total_match = re.search(r"consists of.*?\*\*(\d+)", content)
    total = int(total_match.group(1)) if total_match else 0

    # Pattern for categories in the overview table: "| Unit Tests | ~589 |"
    def extract_category(category_name: str) -> int:
        # Flexible for: | Category | ~123 | OR |Category|~123|
        pattern = rf"\|\s*{category_name}\s*\|\s*~?(\d+)\s*\|"
...
def update_testing_md(
    actual: CategoryTestCounts, dry_run: bool = True, root_path: Optional[Path] = None
) -> list[str]:
    ...
    structure_updates = [
        ...
        (
            r"# Integration tests \(~\d+ tests\)",
            f"# Integration tests (~{actual.integration} tests)",
        ),
        (
            r"# Schema validation \(~\d+ tests\)",
            f"# Schema validation (~{actual.validation} tests)",
        ),
        (
            r"# Guardian tests \(~\d+ tests\)",
            f"# Guardian tests (~{actual.guardian} tests)",
        ),
        (
            r"# Memory system tests \(~\d+ tests\)",
            f"# Memory system tests (~{actual.memory} tests)",
        ),
    ]
...
Examples:
    ...
    parser.add_argument(
        "--sync", action="store_true", help="Perform sync (not just check)"
    )
    parser.add_argument(
        "--dirs", nargs="+", help="Sync artifacts for these directories"
    )
    parser.add_argument(
        "--fast",
        action="store_true",
    ...
    if args.dirs:
        ...
        results = engine.sync_all(
            ...
            artifact_filter=args.artifacts,
    ...
    errors = [
        r
        for r in results
        if "error" in r.message.lower() or "not found" in r.message.lower()
    ]
    ...
    for result in results:
        ...
        if "error" in result.message.lower() or "not found" in result.message.lower():
            status = "[SKIP]"
        print(
            f"  {status} {result.target_file} ({result.target_type}): {result.message}"
        )
