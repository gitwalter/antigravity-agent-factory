...
class ValuesFixer:
    ...
    def fix_agent_pattern(self, pattern_path: Path) -> bool:
        ...
        has_guardian = "guardian" in str(data).lower() or data.get(
            "guardian_awareness", {}
        ).get("enabled", False)
    ...
    def fix_skill_pattern(self, pattern_path: Path) -> bool:
        ...
        has_axiom = any(
            "axiom" in str(r).lower() or "A1" in str(r) for r in important_rules
        )
...
def main():
    ...
    parser.add_argument(
        "--update", action="store_true", help="Update mode (apply fixes)"
    )
