    ...
    def _scan_templates(self) -> None:
        ...
        for template_path in templates_dir.rglob("*"):
            if template_path.is_file():
                ...
                self.nodes[node_id] = DependencyNode(
                    id=node_id, node_type=NodeType.TEMPLATE, path=template_path
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate Factory dependency graph")
    parser.add_argument(
        "--root",
        "-r",
        type=Path,
    ...
    parser.add_argument(
        "--cycles", action="store_true", help="Check for circular dependencies only"
    )
    parser.add_argument(
        "--broken", action="store_true", help="Check for broken references only"
    )
    parser.add_argument(
        "--impact",
        type=str,
    ...
    parser.add_argument(
        "--dependents", type=str, metavar="NODE", help="Show what depends on a node"
    )
    parser.add_argument("--order", action="store_true", help="Show installation order")
    parser.add_argument(
        "--export", type=Path, metavar="FILE", help="Export graph to JSON file"
    )
    ...
    if exit_code == 0:
        print(
            f"[OK] Dependency graph valid ({len(validator.nodes)} nodes, {len(validator.edges)} edges)"
        )
    return exit_code
