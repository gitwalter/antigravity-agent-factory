...
class ValuesAuditor:
    ...
    def audit_blueprint(self, blueprint_path: Path) -> List[AuditResult]:
        ...
        blueprint_id = data.get("metadata", {}).get(
            "blueprintId", blueprint_path.parent.name
        )
        ...
        has_guardian_knowledge = any(
            "guardian" in str(k).lower() or "axiom" in str(k).lower() for k in knowledge
        )
    ...
    def audit_agent_pattern(self, pattern_path: Path) -> List[AuditResult]:
        ...
        has_guardian = (
            "guardian" in str(data).lower()
            or "axiom" in str(data).lower()
            or data.get("guardian_awareness", {}).get("enabled", False)
        )
        results.append(
            AuditResult(
                file_path=str(pattern_path),
    ...
    def audit_skill_pattern(self, pattern_path: Path) -> List[AuditResult]:
        ...
        has_axiom_ref = any(
            "axiom" in str(r).lower() or "A1" in str(r) or "A4" in str(r)
    ...
    def audit_all(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failures": [
                {"file": r.file_path, "check": r.check, "message": r.message}
                for r in failed
            ],
        }
