...
def fix_knowledge(path):
    ...
    if 'title' not in data:
        data['title'] = data.get('metadata', {}).get('name', path.stem).capitalize() + " Knowledge"
        changed = True
    if 'description' not in data:
        data['description'] = data.get('metadata', {}).get('description', "Comprehensive knowledge patterns for " + path.stem)
        changed = True
    ...
    if isinstance(patterns, dict):
        if not patterns:
            patterns['generic_base'] = {
                ...
                "code_example": "// Placeholder for " + path.stem + " implementation details.",
                "best_practices": ["Follow standard conventions", "Keep it simple and clean"]
            }
            ...
            for p_id, p_data in patterns.items():
                ...
                if 'description' not in p_data or len(p_data.get('description', '')) < 20:
                    descr = p_data.get('description', "Pattern " + p_id + " for " + path.stem)
                    p_data['description'] = descr.ljust(20, '.')
                    changed = True
                if 'use_when' not in p_data or len(str(p_data.get('use_when', ''))) < 10:
                    uw = p_data.get('use_when', "When implementing " + p_id)
                    if isinstance(uw, list):
                        uw = ". ".join(str(i) for i in uw)
                    p_data['use_when'] = str(uw).ljust(10, '.')
                    changed = True
                if 'code_example' not in p_data or len(p_data.get('code_example', '')) < 20:
                    ce = p_data.get('code_example', "// Example for " + p_id)
                    p_data['code_example'] = ce.ljust(20, '.')
                    changed = True
                if 'best_practices' not in p_data or not isinstance(p_data['best_practices'], list) or len(p_data['best_practices']) < 2:
                    p_data['best_practices'] = ["Use appropriately for best results.", "Monitor results and optimize."]
                    changed = True
                elif any(len(str(bp)) < 10 for bp in p_data['best_practices']):
                    p_data['best_practices'] = [str(bp).ljust(10, '.') for bp in p_data['best_practices']]
                    changed = True
...
def fix_pattern(path, is_agent=True):
    try:
        with open(path, 'r', encoding='utf-8') as f:
            data = json.load(f)
    except Exception: return # Skip non-json or broken
