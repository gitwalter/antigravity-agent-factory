...
Usage:
    from scripts.git.merge_strategy import MergeEngine, ConflictResolution
...
class MergeEngine:
    ...
    def __init__(self, inventory: RepoInventory, factory_root: Optional[Path] = None):
    ...
    def _create_skill_conflict(self, skill_name: str) -> Optional[Conflict]:
        ...
        """
        existing_path = (
            self.inventory.path / ".agent" / "skills" / skill_name / "SKILL.md"
        )
        if not existing_path.exists():
            return None
    ...
    def should_skip_artifact(
        self, artifact_type: ArtifactType, artifact_name: str
    ) -> bool:
        ...
        return resolution in (ConflictResolution.KEEP_EXISTING, ConflictResolution.SKIP)
...
def merge_json_files(
    existing_content: str, new_content: str, strategy: str = "deep"
) -> MergeResult:
...
if __name__ == "__main__":
    ...
    print("Example:")
    print("  from scripts.git.merge_strategy import MergeEngine")
