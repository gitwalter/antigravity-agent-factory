...
class PreCommitRunner:
    ...
    def __init__(self, root_dir: Path, sync_mode: bool = False, fast_mode: bool = False):
        """
        Initialize runner.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run pre-commit validation scripts"
    )
    parser.add_argument(
        "--sync",
        action="store_true",
        help="Run scripts in sync mode (apply fixes)"
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="Run scripts in check mode (validate only)"
    )
    parser.add_argument(
        "--fast",
        action="store_true",
    ...
    if sync_mode and report.successful > 0:
        if runner.auto_stage():
            print("\n✅ Auto-staged modified files")
    
    if report.failed > 0:
        print(f"\n❌ {report.failed} script(s) failed")
        return 1
