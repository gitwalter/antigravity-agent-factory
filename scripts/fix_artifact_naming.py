...
def main():
    ...
    for file_path in KNOWLEDGE_DIR.glob("*.json"):
        ...
        if old_name != new_name:
            ...
            if new_path.exists():
                logger.warning(f"Target file {new_name} already exists. Overwriting with {old_name}...")
    ...
    if RULES_DIR.exists():
        ...
        for file_path in RULES_DIR.glob("*"):
            if file_path.suffix in [".json", ".yaml", ".md"] and file_path.name != "README.md":
                old_name = file_path.name
                new_name = to_kebab_case(old_name)
                if old_name != new_name:
        ...
        for agent_file in AGENTS_DIR.glob("*.md"):
            try:
                ...
                for old_name, new_name in renames.items():
                    ...
                    # Pattern: [Label](../knowledge/Old Name.json)
    ...
    if SKILLS_DIR.exists():
        for skill_file in SKILLS_DIR.glob("**/SKILL.md"):
            try:
                ...
                if modified:
                    logger.info(f"Updating references in skill: {skill_file.relative_to(SKILLS_DIR)}")
