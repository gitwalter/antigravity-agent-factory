...
STACK_CONFIGS = {
    ...
    "ai": {
        ...
        "crewai": {
            ...
            "dependencies": {"prod": ["crewai", "crewai-tools"], "dev": ["pytest"]},
            "gitignore_patterns": ["__pycache__/", "*.pyc", ".env", ".venv/"],
        },
        "huggingface": {
            "package_manager": "pip",
...
def generate_package_json(config: WorkshopConfig, stack_config: Dict[str, Any]):
    ...
    package = {
        ...
        "main": "index.js",
        "scripts": {"test": 'echo "Tests not yet configured" && exit 0'},
...
"""
    ...
    for phase in workshop.get("phases", []):
        content += (
            f"| {phase.get('name', '')} | {phase.get('duration_minutes', 0)} min |\n"
        )
...
def export_workshop(workshop_id: str, target_dir: str, factory_root: str = None):
    ...
    generate_cursorrules(config, stack_config)
    ...
    print(f"{'=' * 60}")
    print("\nNext steps:")
    print(f"  1. Open {target_path} in Antigravity")
    print("  2. Install dependencies")
    print("  3. Read README.md to begin the workshop")
    print("\nHappy learning!")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print(
            "Example: python export_workshop.py L1_ethereum_fundamentals c:/App/learning/ethereum"
        )
        sys.exit(1)
