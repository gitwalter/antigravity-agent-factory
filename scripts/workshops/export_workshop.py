...
Artifacts Generated:
    ...
    - Antigravity rules (.agentrules, .agent/rules/)    - Knowledge files (copied from factory)
...
def generate_cursorrules(config: WorkshopConfig, stack_config: Dict[str, Any]):
    """Generate .agentrules file for the project."""    workshop = config.workshop_data
...
"""
    ...
    cursorrules_path = config.target_dir / ".agentrules"
...
def generate_technology_rules(config: WorkshopConfig, stack_config: Dict[str, Any]):
    """Generate technology-specific rules in .agent/rules/."""    workshop = config.workshop_data
    technology = workshop.get("technology", {})
...
.agentrules         # Project-level AI rulesREADME.md            # This file
```
...
def copy_knowledge_files(config: WorkshopConfig):
    ...
    knowledge_dir = config.target_dir / ".agent" / "knowledge"    knowledge_dir.mkdir(parents=True, exist_ok=True)
...
"""
    ...
    context_path = config.target_dir / ".agent" / "WORKSHOP_CONTEXT.md"    with open(context_path, 'w', encoding='utf-8') as f:
        f.write(content)
...
def export_workshop(workshop_id: str, target_dir: str, factory_root: str = None):
    ...
    generate_cursorrules(config, stack_config)    generate_technology_rules(config, stack_config)
    ...
    print(f"{'='*60}")
    print(f"\nNext steps:")
    print(f"  1. Open {target_path} in Antigravity")    print(f"  2. Install dependencies")
