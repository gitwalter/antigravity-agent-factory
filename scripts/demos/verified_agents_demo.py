...
def demo_basic_communication(context: SocietyContext) -> tuple:
    ...
    worker = AgentSocietyBridge(
        agent_id="worker-1", agent_type="executor", context=context, name="Worker Agent"
    )
    ...
    result = orchestrator.send_message(
        ...
        payload={"task": "analyze_code", "file": "main.py", "scope": "security"},
...
def demo_contracts(
    ...
    print("\n  Signing contract...")
    print("    [OK] Orchestrator signed (via create)")
    worker.sign_contract(contract)
    print("    [OK] Worker signed")
...
def demo_event_store(context: SocietyContext) -> None:
    ...
    for event in events[-5:]:
        print(
            f"    seq={event.sequence} | {event.agent.id} | {event.action.type.value}"
        )
...
def main():
    ...
    print("    [OK] Event store initialized")
    print("    [OK] Axiom monitor active (6 verifiers)")
    print("    [OK] Contract registry ready")
    print("    [OK] Reputation system online")
