...
def run_command(command: List[str], cwd: str = None, check: bool = True) -> Tuple[int, str, str]:
    ...
    if not shutil.which(git_cmd) and shutil.which("git"):
        git_cmd = "git" # Fallback to standard git if absolute path fails but git is in PATH
...
def add_upstream_remote():
    ...
    code, _, stderr = run_command(["git", "remote", "add", UPSTREAM_REMOTE_NAME, UPSTREAM_URL])
    if code != 0:
...
def resolve_content_conflict(content: str) -> str:
    ...
    def replacer(match):
        ...
        resolved = resolved.replace("Cursor", "Antigravity") # Be careful with this one, might be too aggressive?
...
def resolve_conflicts():
    ...
    for filepath in conflicted_files:
        ...
        try:
            ...
            if "<<<<<<< HEAD" not in resolved_content:
                ...
                print(f"  Failed to fully resolve: {filepath}. Keeping local version (ours).")
    ...
    if resolved_count == len(conflicted_files):
        ...
        run_command(["git", "commit", "-m", "chore: sync upstream with intelligent branding preservation"])
    else:
...
def merge_upstream(branch: str, dry_run: bool = False):
    ...
    code, stdout, stderr = run_command(["git", "merge", upstream_ref, "--allow-unrelated-histories"])
    
    if code != 0:
        print(f"Merge encountered conflicts. Starting intelligent resolution...")
        resolve_conflicts()
    else:
...
def prune_docs():
    ...
    dirs_to_remove = [
        "agents", "articles", "design", "examples", "guides", 
        "moments", "pm-system", "research", "workshops"
    ]
    
    # Files to remove
    files_to_remove = [
        "ASP_VALUE_PROPOSITION.md", "COMPLETE_GUIDE.md", "CULTURE_AND_VALUES.md", 
        "FACTORY_REFERENCE.md", "FIRST_WEEK_GUIDE.md", "KNOWLEDGE_EVOLUTION.md", 
        "LAYERED_ARCHITECTURE.md", "LAYERED_ONBOARDING_CONCEPT.md", 
        "LEARNING_WORKSHOPS.md", "MCP-SERVERS.md", "MEMORY_SYSTEM.md", 
        "ONBOARDING_GUIDE.md", "PATH_CONFIGURATION.md", "PREREQUISITES.md", 
        "SAP_GROUNDING_DESIGN.md", "SOCIETY_USAGE.md", "TEAM_WORKSHOP_GUIDE.md", 
        "USAGE_GUIDE.md", "VERIFICATION.md", "WORKFLOW_AUTHORING.md"
    ]
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Simulate the sync process without making changes.")
    parser.add_argument("--branch", default=DEFAULT_BRANCH, help=f"Upstream branch to sync with (default: {DEFAULT_BRANCH})")
