            ...
            if os.path.splitext(f)[1] in scan_exts:
                ...
                try:
                    ...
                    for match in reversed(matches):
                        ...
                        # Skip external links
                        if link.startswith(("http", "mailto", "tel", "file:")):
                            continue
                        ...
                        if current_rel.exists() or current_root.exists():
                            continue
                        ...
                        if "configuration" in target_name.lower():
                            search_names.append("configuration.md")
                        ...
                        if best_target:
                            fixed_rel = os.path.relpath(
                                best_target, file_path.parent
                            ).replace("\\", "/")
                            if "/" not in fixed_rel and not fixed_rel.startswith("."):
                                fixed_rel = f"./{fixed_rel}"
                            if anchor:
                                fixed_rel += f"#{anchor}"

                            print(
                                f"  FIX: {file_path.relative_to(ROOT_DIR)}: {link} -> {fixed_rel}"
                            )
