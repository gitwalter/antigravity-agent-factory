...
def fix_skill(path):
    ...
    for pattern, replacement in mappings.items():
        new_content = re.sub(
            pattern, replacement, new_content, flags=re.MULTILINE | re.IGNORECASE
        )
    ...
    if not re.search(r"^##+\s+When to Use", new_content, re.MULTILINE | re.IGNORECASE):
        ...
        insertion = (
            "## When to Use\n\nThis skill should be used when completing tasks related to "
            + path.parent.name.replace("-", " ")
            + ".\n\n"
        )
        if first_section_match:
            new_content = (
                new_content[: first_section_match.start()]
                + insertion
                + new_content[first_section_match.start() :]
            )
        else:
    ...
    new_content = re.sub(
        r"(## Process\s*\n\s*\n)(?=\s*###)",
        r"\1Follow these procedures to implement the capability:\n\n",
        new_content,
    )
        ...
        if len(parts) >= 3:
            ...
            try:
                ...
                if current_name != dir_name:
                    print(
                        f"Correcting name mismatch for {path}: {current_name} -> {dir_name}"
                    )
                    fm["name"] = dir_name

                fm["type"] = "skill"
                fm["description"] = fm.get(
                    "description", f"Capability for {dir_name.replace('-', ' ')}."
                )
