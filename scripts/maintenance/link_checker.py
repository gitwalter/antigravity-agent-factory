...
class LinkChecker:
    ...
    def build_index(self):
        ...
        ignore_dirs = {".git", ".venv", "node_modules", ".pytest_cache", ".ruff_cache", "__pycache__", "tmp"}
    ...
    def find_best_match(self, source_file, link_text):
        ...
        for cand in candidates:
            ...
            if common > max_common:
                ...
                if abs(len(cand.parts) - len(source_parts)) < abs(len(best_candidate.parts) - len(source_parts)):
                    best_candidate = cand
    ...
    def resolve_to_relative(self, source_file, target_file):
        try:
            ...
            if '/' not in rel and not rel.startswith('.'): rel = f"./{rel}"
            return rel
                ...
                for match in reversed(matches):
                    ...
                    if self.is_internal(link):
                        if link.startswith("file:"):
                            path_str = unquote(link.replace("file:///", "").replace("file://", ""))
                            # Handle windows paths potentially with drive letters
                            if ":" in path_str and path_str.startswith("/"): path_str = path_str[1:]
                            ...
                            root_path = (self.root_dir / clean_link.lstrip('/')).resolve()
                            is_valid = rel_path.exists() or root_path.exists()
                        
                        # Special handling for directory links
                        if not is_valid and not link.startswith("file:") and clean_link.endswith('/'):
                        ...
                        if needs_fix and self.fix_mode:
                            ...
                            if target:
                                ...
                                if fixed and fixed != link:
                                    if anchor: fixed += f"#{anchor}"
                                    print(f"  [FIX] {file_path.relative_to(self.root_dir)}: {link} -> {fixed}")
                            ...
                            if root_path.exists():
                                ...
                                if fixed and fixed != link:
                                    if anchor: fixed += f"#{anchor}"
                                    print(f"  [NORM] {file_path.relative_to(self.root_dir)}: {link} -> {fixed}")
                                    start, end = match.span(2)
                                    content = content[:start] + fixed + content[end:]
                                    modified = True
                                    self.fixed_links_count += 1
    ...
    async def run(self, specific_files=None):
        ...
        if specific_files:
            ...
            ignore_pattern = re.compile(r'\.(git|venv|pytest_cache|ruff_cache)|node_modules|tmp|__pycache__')
            for root, _, files in os.walk(self.root_dir):
                if ignore_pattern.search(root): continue
                for f in files:
...
async def main():
    ...
    parser.add_argument("--normalize", action="store_true", help="Normalize valid root-based links to relative")
