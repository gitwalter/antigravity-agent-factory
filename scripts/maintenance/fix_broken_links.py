...
def fix_text_content(content: str, file_path: Path, root_dir: Path) -> str:
    ...
    def replace_link(match):
        ...
        if target_path_root.exists():
            ...
            try:
                ...
                if rel_path != clean_path_str:
                    print(
                        f"Fixing path in {file_path.name}: '{path_str}' -> '{rel_path}{anchor}'"
                    )
                    return f"**{text}**"
                return match.group(0)
            except ValueError:
        ...
        if clean_path_str.startswith("knowledge/"):
            try:
                ...
                if rel_path != clean_path_str:
                    print(
                        f"Forcing relative path in {file_path.name}: '{path_str}' -> '{rel_path}{anchor}'"
                    )
                    return f"**{text}**"
            except ValueError:
                pass
        ...
        kebab_name = to_kebab_case(
            to_kebab_case(path_obj.name)
        )  # Double ensure? Just utilize the function.
        ...
        if target_path_kebab_root.exists():
            try:
                ...
                rel_path = rel_path.replace("\\", "/")
                print(
                    f"Fixing kebab path in {file_path.name}: '{path_str}' -> '{rel_path}{anchor}'"
                )
                return f"**{text}**"
            except ValueError:
                pass
    ...
    def replace_wiki(match):
        ...
        if inner != kebab_inner:
            ...
            print(
                f"Fixing wikilink in {file_path.name}: '[[{inner}]]' -> '[[{kebab_inner}]]'"
            )
...
def main():
    ...
    md_dirs = [root / ".agent/agents", root / ".agent/skills"]
