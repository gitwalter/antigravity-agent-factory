...
def fix_pattern_schema(factory_root: Path):
    ...
    if agents_dir.exists():
        for pattern_file in agents_dir.glob("*.json"):
            if (
                pattern_file.name.endswith("-pattern.json")
                or pattern_file.name == "schema.json"
            ):
                continue
            ...
            try:
                ...
                if "domain" not in fm:
                    ...
                    if cat == "core":
                        ...
                        fm["domain"] = "development"

                if json.dumps(data) != json.dumps(
                    json.loads(content)
                ):  # Check if changed
    ...
    if skills_dir.exists():
        for pattern_file in skills_dir.glob("*.json"):
            ...
            try:
                ...
                if "version" not in fm:
                    fm["version"] = data.get("version", "1.0.0")

                # Check/Fix Category
                current_cat = fm.get("category") or data.get("metadata", {}).get(
                    "category", "core"
                )
                ...
                if current_cat == "ai-development":
                    current_cat = "ai-ml"
                if current_cat == "quality":
                    current_cat = "testing"
                if current_cat == "verification":
                    current_cat = (
                        "testing"  # or security? For security-audit, maybe security.
                    )

                if "security" in pattern_file.name and current_cat == "testing":
                    ...
                    if field not in fm:
                        ...
                        if val:
                            ...
                            if field in ["tools", "templates"]:
                                ...
                                fm[field] = [
                                    "none.json"
                                ]  # Schema requires .json extension
                            else:
                                # agents, related_skills minItems 1
                                if field == "agents":
                                    fm[field] = ["code-reviewer"]  # fallback
                                if field == "related_skills":
                                    fm[field] = ["onboarding-flow"]  # fallback
