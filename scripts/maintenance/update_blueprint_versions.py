...
class VersionUpdater:
    ...
    def update_framework_versions(
        self, blueprint: Dict, versions: Dict[str, str]
    ) -> Tuple[Dict, List[str]]:
    ...
    def update_llm_models(
        self, blueprint: Dict, versions: Dict
    ) -> Tuple[Dict, List[str]]:
            ...
            if provider_name_lower in llm_models:
                ...
                for model in old_models:
                    if model not in new_models:
                        changes.append(
                            f"  {provider['name']}: Removed outdated model {model}"
                        )
    ...
    def update_knowledge_files(
        self, knowledge_dir: Path, versions: Dict[str, str]
    ) -> int:
            ...
            try:
                    ...
                    for key, value in content["versions"].items():
                        if key in versions:
                            ...
                            if old_version != new_version:
                                ...
                                logger.info(
                                    f"  {knowledge_file.name}: {key} {old_version} → {new_version}"
                                )
                                updated = True
                ...
                if updated:
                    if "metadata" not in content:
                        content["metadata"] = {}
                    content["metadata"]["last_version_update"] = (
    ...
    def update_blueprint_file(
        self, blueprint_path: Path, versions: Dict[str, str]
    ) -> bool:
        ...
        try:
            ...
            # Update frameworks
            blueprint, framework_changes = self.update_framework_versions(
                blueprint, versions
            )
    ...
    def update_all_blueprints(
        self, blueprints_dir: Path, blueprint_id: Optional[str] = None
    ) -> None:
        ...
        logger.info("\n" + "=" * 80)
        logger.info(
            f"Summary: {updated_count}/{len(blueprint_files)} blueprint(s) updated"
        )
        if self.dry_run:
