...
IMPORTANT_DOCS = {
    "README.md", "QUICKSTART.md", "CHANGELOG.md", "LICENSE.md",
    "SYSTEM_BLUEPRINT.md", "BLUEPRINT_VERSION_MANAGEMENT.md",
    "GETTING_STARTED.md", "CONFIGURATION.md", "INSTALLATION.md",
    "TESTING.md", "USER_GUIDE.md", "TROUBLESHOOTING.md",
    "ASP_VALUE_PROPOSITION.md", "EXTENSION_GUIDE.md", "TEST_CATALOG.md"
}
...
def master_repair():
    ...
    for root, _, files in os.walk(DOCS_DIR):
        for f in files:
            ...
            target = clean_name(f)
            if f in MANUAL_FIXES: target = MANUAL_FIXES[f]
            if f != target:
    ...
    for f in os.listdir(ROOT_DIR):
        if f.upper() in {s.upper() for s in ROOT_CAPS}:
            ...
            if f != target:
                full_rename_map[os.path.join(ROOT_DIR, f)] = os.path.join(ROOT_DIR, target)
    ...
    if not full_rename_map:
        ...
        for old, new in full_rename_map.items():
            ...
            if old_p.name.lower() == new_p.name.lower() and old_p.name != new_p.name:
                ...
                if os.path.exists(new) and old != new: os.remove(new)
                os.rename(old, new)
