        ...
        "questions": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "type": {
                        "type": "string",
                        "enum": ["single", "multiple", "conditional"],
                    },
...
class TestAdapterInterfaceSchema:
    ...
    def test_interface_has_operations(self, pm_adapters_dir):
        ...
        for section in required_sections:
            assert (
                section in interface_props
            ), f"Interface should define '{section}' section"
            assert isinstance(
                interface_props[section], dict
        ...
        for op in required_work_item_ops:
            assert op in work_items_props, f"WorkItems should define '{op}' operation"
...
class TestBackendAdaptersSchema:
    ...
    def test_all_adapters_valid_json(self, pm_adapters_dir):
        ...
        adapter_files = [
            f
            for f in pm_adapters_dir.glob("*-adapter.json")
        ...
        for adapter_file in adapter_files:
            ...
            for section in required_sections:
                if section not in mappings_sections:
                    errors.append(
                        f"{adapter_file.name}: Mappings should define '{section}' section"
                    )
    ...
    def test_adapters_implement_interface_operations(self, pm_adapters_dir):
        ...
        for adapter_file in adapter_files:
            ...
            elif "properties" in adapter_data and "mappings" in adapter_data.get(
                "properties", {}
            ):
            ...
            for section in required_sections:
                if section in mappings:
                    ...
                    if section in ["workItems", "planning"]:
                        ...
                        mapped_ops = [
                            op for op in section_required_ops if op in section_mappings
                        ]
                        assert (
                            len(mapped_ops) > 0
...
class TestMethodologyDefaultsSchema:
    ...
    def test_all_defaults_valid_json(self, pm_defaults_dir):
        ...
        for defaults_file in defaults_files:
            ...
            assert isinstance(
                data, dict
            ), f"{defaults_file.name} should be a dictionary"
...
class TestMetricsSchema:
    ...
    def test_metrics_have_required_fields(self, knowledge_dir, validator):
        ...
        for metric_id, metric in metrics.items():
            ...
            assert (
                "formula" in metric
            ), f"Metric '{metric_id}' should have 'formula' field"
            assert "unit" in metric, f"Metric '{metric_id}' should have 'unit' field"
            assert (
                "category" in metric
            ), f"Metric '{metric_id}' should have 'category' field"
    ...
    def test_all_categories_covered(self, knowledge_dir):
        ...
        if "metric_categories" in data:
            ...
            for category in required_categories:
                assert (
                    category in category_definitions
