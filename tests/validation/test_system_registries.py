...
def test_agent_registry_consistency():
    ...
    for agent in registry["agents"]:
        # Check DID format
        assert agent["did"].startswith("did:agent:"), f"Invalid DID for {agent['agent_id']}"
        ...
        for skill_id in agent["skills"]:
            ...
            assert bare_skill_id in skill_catalog, f"Agent {agent['agent_id']} references unregistered skill: {skill_id}"
        ...
        if "path" in agent:
            ...
            assert agent_path.exists(), f"Agent definition file missing for {agent['agent_id']}: {agent_path}"
...
def test_skill_catalog_paths():
    ...
    for skill_id, skill in skill_catalog.items():
        ...
        path_str = skill[path_key].replace("{directories.skills}", ".agent/skills").replace("{directories.patterns}", ".agent/patterns")
        path_str = path_str.replace("/", os.sep).replace("\\", os.sep)
        skill_path = root / path_str
        assert skill_path.exists(), f"Skill definition missing for {skill_id}: {skill_path} (from {path_key})"
...
def test_graph_integrity():
    ...
    for edge in graph["edges"]:
        ...
        for key in ["from", "to"]:
            node_id = edge[key]
            ignored_targets = ["template:", "pattern:", "skill:", "agent:", "knowledge:"]
