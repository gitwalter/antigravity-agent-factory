...
class TestBlueprintCompleteness:
        ...
        for blueprint_dir in blueprints_dir.iterdir():
            if blueprint_dir.is_dir():
                ...
                if blueprint_file.exists():
                    ...
                    agent_ids = {agent.get("patternId") if isinstance(agent, dict) else agent 
                                 for agent in agents}
        ...
        for blueprint_dir in blueprints_dir.iterdir():
            if blueprint_dir.is_dir():
                ...
                if blueprint_file.exists():
                    ...
                    skill_ids = {skill.get("patternId") if isinstance(skill, dict) else skill 
                                 for skill in skills}
    ...
    def test_all_blueprints_have_all_standard_agents(self, blueprints_dir):
        ...
        assert len(missing_agents) == 0, \
            f"Blueprints missing standard agents:\n" + \
            "\n".join(f"  {bp}: {', '.join(missing)}" 
    ...
    def test_all_blueprints_have_all_standard_skills(self, blueprints_dir):
        ...
        assert len(missing_skills) == 0, \
            f"Blueprints missing standard skills:\n" + \
            "\n".join(f"  {bp}: {', '.join(missing)}" 
...
class TestPatternFileExistence:
    ...
    def test_debug_conductor_project_pattern_exists(self, patterns_dir):
        ...
        assert pattern_file.exists(), \
            f"Pattern file does not exist: {pattern_file}"
        ...
        with open(pattern_file, 'r', encoding='utf-8') as f:
            try:
                ...
                pytest.fail(f"Pattern file is not valid JSON: {pattern_file}\nError: {e}")
    ...
    def test_all_agent_patterns_exist(self, patterns_dir):
        ...
        assert len(missing) == 0, \
            f"Missing agent pattern files: {', '.join(missing)}"
        assert len(invalid) == 0, \
            f"Invalid agent pattern files: {', '.join(invalid)}"
    ...
    def test_all_skill_patterns_exist(self, patterns_dir):
        ...
        assert len(missing) == 0, \
            f"Missing skill pattern files: {', '.join(missing)}"
        assert len(invalid) == 0, \
            f"Invalid skill pattern files: {', '.join(invalid)}"
...
class TestPatternSchemaValidation:
    ...
    def test_patterns_have_required_metadata(self, patterns_dir):
        ...
        assert len(errors) == 0, \
            f"Pattern schema validation errors:\n" + "\n".join(f"  - {e}" for e in errors)
    ...
    def test_patterns_have_frontmatter_with_name_and_description(self, patterns_dir):
        ...
        assert len(errors) == 0, \
            f"Pattern frontmatter validation errors:\n" + "\n".join(f"  - {e}" for e in errors)
    ...
    def test_patterns_have_sections(self, patterns_dir):
        ...
        assert len(errors) == 0, \
            f"Pattern sections validation errors:\n" + "\n".join(f"  - {e}" for e in errors)
...
class TestGenerationScriptValidation:
    ...
    def test_generate_project_includes_standard_agents_list(self, factory_root):
        ...
        assert script_path.exists(), \
            f"Generation script does not exist: {script_path}"
        
        with open(script_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Check for standard_agents list definition
        assert "standard_agents" in content, \
            "generate_project.py does not include standard_agents list"
        
        # Check that it includes debug-conductor-project
        assert "debug-conductor-project" in content or "'debug-conductor-project'" in content or '"debug-conductor-project"' in content, \
    ...
    def test_generate_project_includes_standard_skills_list(self, factory_root):
        ...
        assert "standard_skills" in content, \
        ...
        assert len(missing) == 0, \
            f"standard_skills list missing: {', '.join(missing)}"
    ...
    def test_generate_project_adds_standard_agents(self, factory_root):
        ...
        for i, line in enumerate(lines):
            ...
            if found_standard_agents and ("agents_to_generate" in line or "append" in line.lower()):
                found_append = True
                break
        
        assert found_standard_agents, \
    ...
    def test_generate_project_adds_standard_skills(self, factory_root):
        ...
        for i, line in enumerate(lines):
            ...
            if found_standard_skills and ("skills_to_generate" in line or "append" in line.lower()):
                found_append = True
                break
        
        assert found_standard_skills, \
