...
class TestTemplateSyntax:
    ...
    def test_jinja2_available(self):
        ...
        if not JINJA2_AVAILABLE:
            pytest.skip(
                "Jinja2 library not installed. Install with: pip install jinja2"
            )
    ...
    def test_templates_have_valid_syntax(
        self, all_template_files: List[Path], jinja_env
    ):
        ...
        for template_file in all_template_files:
            try:
                ...
                errors.append(
                    f"{rel_path}: Syntax error at line {e.lineno}: {e.message}"
                )
    ...
    def test_templates_can_be_compiled(self, all_template_files: List[Path], jinja_env):
        ...
        for template_file in all_template_files:
            try:
                ...
                rel_path = template_file.relative_to(template_file.parent.parent.parent)
                errors.append(
                    f"{rel_path}: Compilation error at line {e.lineno}: {e.message}"
                )
            except Exception as e:
                rel_path = template_file.relative_to(template_file.parent.parent.parent)
    ...
    def test_templates_have_valid_filters(
        self, all_template_files: List[Path], jinja_env
    ):
        ...
        for template_file in all_template_files:
            try:
                ...
                for filter_name in filters:
                    if filter_name not in common_filters and not filter_name.startswith(
                        "_"
                    ):
...
class TestTemplateVariables:
    ...
    def test_templates_extract_undefined_variables(
        self, all_template_files: List[Path], jinja_env
    ):
    ...
    def test_templates_use_default_filter_for_variables(
        self, all_template_files: List[Path]
    ):
        ...
        for template_file in all_template_files:
            ...
            if undefined_vars:
                ...
                rel_path = template_file.relative_to(template_file.parent.parent.parent)
                warnings.append(
                    f"{rel_path}: Variables without defaults: {', '.join(set(undefined_vars))}"
                )
    ...
    def test_templates_have_reasonable_length(self, all_template_files: List[Path]):
        ...
        for template_file in all_template_files:
            ...
            if len(content.strip()) < 10:
                rel_path = template_file.relative_to(template_file.parent.parent.parent)
                errors.append(
                    f"{rel_path}: Template content too short ({len(content)} chars)"
                )
    ...
    def test_templates_use_j2_extension(self, templates_dir: Path):
        ...
        for tmpl_file in templates_dir.rglob("*.tmpl"):
            ...
            if "{{" in content or "{%" in content:
                ...
                errors.append(
                    f"{rel_path}: Contains Jinja2 syntax but uses .tmpl extension (consider .j2)"
                )
