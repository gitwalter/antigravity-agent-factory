...
""", encoding="utf-8")
        ...
        assert is_valid is False, "Comma-separated strings should be detected as invalid"
        assert error is not None
        # PyYAML reports "found ','" or our custom check reports "comma" or "array"
        error_lower = error.lower()
        assert any(x in error_lower for x in ["comma", "array", "','"]), \
...
""", encoding="utf-8")
    ...
    def test_validate_file_without_frontmatter(self, tmp_path):
        ...
        test_file.write_text("""# Just a Document
