...
class TestEmbeddingService:
    ...
    def service(self):
        ...
        try:
            ...
            network_indicators = ['timeout', 'connection', 'network', 'readtimeout', 'httpx']
            if any(indicator in error_str or indicator in error_type for indicator in network_indicators):
                pytest.skip(f"Skipping due to network issue during model download: {type(e).__name__}: {e}")
    ...
    def test_similar_texts_have_high_similarity(self, service):
        ...
        score = service.similarity(
            ...
            ["Testing Python code with pytest framework"]
        )
