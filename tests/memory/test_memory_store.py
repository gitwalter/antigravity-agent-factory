...
class TestMemoryStore:
    ...
    def test_add_memory_to_different_types(self, store):
        ...
        semantic_id = store.add_memory(
            "Use black for formatting", {"source": "test"}, "semantic"
        )

        # Episodic memory
        episodic_id = store.add_memory(
            "User mentioned pytest", {"source": "observation"}, "episodic"
        )
    ...
    def test_get_memory_by_id(self, store):
        ...
        memory_id = store.add_memory("Test content", {"source": "test"}, "semantic")
    ...
    def test_delete_memory(self, store):
        ...
        memory_id = store.add_memory("To be deleted", {"source": "test"}, "semantic")
    ...
    def test_search_with_threshold(self, store):
        ...
        results = store.search(
            "Python unit testing", memory_type="semantic", k=5, threshold=0.5
        )
    ...
    def test_get_relevant_context(self, store):
        ...
        store.add_memory(
            "Use black for code formatting", {"source": "user_teaching"}, "semantic"
        )
...
class TestMemoryProposalOperations:
    ...
    def test_is_similar_to_rejected(self, store):
        ...
        is_similar = store.is_similar_to_rejected(
            "Use unittest for Python testing", threshold=0.8
        )
        ...
        is_similar = store.is_similar_to_rejected(
            "Cooking pasta recipes", threshold=0.8
        )
