...
class TestAgentContract:
    ...
    def create_sample_contract(self, signed: bool = True) -> AgentContract:
        ...
        contract = AgentContract(
            ...
            obligations={
                "provider": [Obligation(trigger="request", action="respond", parameters={})],
            },
            prohibitions={
                "all_parties": [Prohibition(action="harm", reason="Must not cause harm")],
            },
...
class TestContractVerifier:
    ...
    def setup_method(self):
        ...
        self.contract = AgentContract(
            ...
            prohibitions={
                "all_parties": [Prohibition(action="delete_data", reason="Cannot delete")],
            },
    ...
    def test_verify_prohibited_action(self):
        ...
        has_prohibition = any(v.type == ViolationType.PROHIBITION for v in result.violations)
        assert has_prohibition, f"Expected PROHIBITION violation, got: {[v.type for v in result.violations]}"
    ...
    def test_verify_message_no_contract(self):
        ...
        result = self.verifier.verify_message("unknown-sender", "unknown-receiver", message)
    ...
    def test_verify_message_lacks_capability(self):
        ...
        assert result.status == ContractStatus.VIOLATION
        assert result.has_violations()
        has_capability_violation = any(v.type == ViolationType.CAPABILITY for v in result.violations)
    ...
    def test_fulfill_obligation_success(self):
        ...
        contract = AgentContract(
            ...
            obligations={
                "worker": [Obligation(trigger="request", action="respond", parameters={})],
            },
        ...
        # Fulfill it
        result = self.verifier.fulfill_obligation("test-contract-obligation", "agent-1", "respond")
    ...
    def test_fulfill_obligation_not_party(self):
        ...
        result = self.verifier.fulfill_obligation("test-contract-obligation", "unknown-agent", "respond")
        assert result is False
    ...
    def test_fulfill_obligation_no_matching_action(self):
        ...
        result = self.verifier.fulfill_obligation("test-contract-obligation", "agent-1", "wrong_action")
        assert result is False
    ...
    def test_check_pending_obligations_not_party(self):
        ...
        pending = self.verifier.check_pending_obligations("test-contract-obligation", "unknown-agent")
        assert pending == []
    ...
    def test_check_pending_obligations_timeout_exceeded(self):
        ...
        pending = self.verifier.check_pending_obligations("test-contract-obligation", "agent-1")
