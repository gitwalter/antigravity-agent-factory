...
class TestUpdatePatternsExist:
    ...
    def test_factory_updates_agent_pattern_valid_json(self, patterns_dir: Path):
        ...
        if pattern_path.exists():
            ...
            assert "patternId" in data.get(
                "metadata", {}
            ), "Pattern must have patternId"
    ...
    def test_project_info_template_contains_placeholders(self, templates_dir: Path):
        ...
        if template_path.exists():
            ...
            has_blueprint_placeholder = (
                ...
                or "{{blueprint_id}}" in content
                or "${blueprint_id}" in content
                or "{{ blueprint_id }}" in content
            )
            has_factory_version_placeholder = (
                "{FACTORY_VERSION}" in content
                or "{{factory_version}}" in content
                or "${factory_version}" in content
                or "{{ factory_version }}" in content
            )

            assert (
                has_blueprint_placeholder or "blueprint" in content.lower()
            ), "Template must have blueprint_id placeholder"
            assert (
                has_factory_version_placeholder or "factory" in content.lower()
...
class TestFactoryUpdatesFeed:
    ...
    def test_factory_updates_has_feed_url(self, knowledge_dir: Path):
        ...
        if feed_path.exists():
            ...
            assert (
                "github" in feed_url.lower() or "http" in feed_url.lower()
...
class TestUpdateFiltering:
    ...
    def test_filter_updates_by_specific_blueprint(
        self, sample_factory_updates_feed: Dict[str, Any]
    ):
    ...
    def test_filter_updates_excludes_other_blueprints(
        self, sample_factory_updates_feed: Dict[str, Any]
    ):
    ...
    def test_filter_updates_web_blueprint(
        self, sample_factory_updates_feed: Dict[str, Any]
    ):
    ...
    def test_filter_handles_all_keyword(
        self, sample_factory_updates_feed: Dict[str, Any]
    ):
...
class TestGeneratorIncludesUpdateComponents:
    ...
    def test_generator_has_factory_updates_in_standard_agents(self, factory_root: Path):
        ...
        has_factory_updates = (
            "'factory-updates'" in content or '"factory-updates"' in content
        )

        # For now, just verify the generator exists and has standard_agents
        has_standard_agents = "standard_agents" in content

        assert has_standard_agents, "generate_project.py must have standard_agents list"
    ...
    def test_generator_has_receive_updates_in_standard_skills(self, factory_root: Path):
        ...
        has_receive_updates = (
            "'receive-updates'" in content or '"receive-updates"' in content
        )

        has_standard_skills = "standard_skills" in content

        assert has_standard_skills, "generate_project.py must have standard_skills list"
    ...
    def test_generator_has_project_info_generation(self, factory_root: Path):
        ...
        has_project_info_logic = "project-info" in content or "project_info" in content
...
class TestGeneratedProjectUpdateInfrastructure:
    ...
    def test_project_info_blueprint_id_matches(
        self, generated_project_path: Path, update_config: ProjectConfig
    ):
        ...
        if project_info_path.exists():
            ...
            assert (
                blueprint_id == update_config.blueprint_id
    ...
    def test_factory_updates_agent_content(self, generated_project_path: Path):
        ...
        if agent_path.exists():
            ...
            assert "update" in content.lower(), "Agent must handle update requests"

            # Must reference project-info.json
            assert (
                "project-info" in content.lower() or "project_info" in content.lower()
    ...
    def test_receive_updates_skill_content(self, generated_project_path: Path):
        ...
        if skill_path.exists():
            ...
            assert (
                "fetch" in content.lower() or "download" in content.lower()
            ), "Skill must instruct how to fetch updates"

            # Must reference factory-updates.json feed
            assert (
                "factory-updates" in content.lower()
...
class TestUpdateApplicationLogic:
    ...
    def test_backup_before_update(self, tmp_path: Path):
        ...
        backup_file = backup_dir / "patterns.json.backup"
...
class TestUpdateSystemIntegration:
    ...
    def test_update_files_referenced_in_feed_exist(
        self, knowledge_dir: Path, factory_root: Path
    ):
                ...
                for file_ref in update.get("files", []):
                    ...
                    if source:
                        ...
                        if not source.endswith(".tmpl"):
                            assert source_path.exists(), f"Update {update['id']} references missing file: {source}"
...
class TestRollbackFunctionality:
    ...
    def test_rollback_updates_installed_updates_list(self, tmp_path: Path):
        ...
        current["installed_updates"] = [
            u for u in current["installed_updates"] if u["id"] != "update-2"
        ]
