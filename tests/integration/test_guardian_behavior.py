...
class TestGuardianIntendedBehavior:
    ...
    def test_level_0_safe_command_returns_flow(self):
        ...
        for cmd in safe_commands:
            ...
            assert (
                result.level == 0
            ), f"'{cmd}' should be Level 0 (Flow), got Level {result.level}"
    ...
    def test_level_2_sensitive_file_pauses(self):
        ...
        for cmd_prefix, path in sensitive_files:
            ...
            assert (
                result.level >= 2
            ), f"'{cmd_prefix} {path}' should be Level 2+ (Pause)"
            assert (
                not result.passed
            ), f"'{cmd_prefix} {path}' should require user attention"
    ...
    def test_level_4_high_severity_secrets_protect(self):
        ...
        for content in high_severity_secrets:
            ...
            assert level == 4, "High severity secrets must be Level 4 (Protect)"
...
class TestGuardianMessaging:
    ...
    def test_block_messages_include_recommendations(self):
        ...
        assert any(
            "backup" in r.lower() or "review" in r.lower() or "alternative" in r.lower()
    ...
    def test_caution_messages_are_informative(self):
        ...
        assert (
            "caution" in str(report.details).lower()
            or "sensitive" in str(report.details).lower()
...
class TestGuardianAxiomAlignment:
    ...
    def test_consistency_a5_flagged(self):
        ...
        assert "A5" in str(
            report.details
        ), "Instruction override should cite A5 (Consistency)"
...
class TestGuardianNoFalsePositives:
    ...
    def test_documentation_examples_not_blocked(self):
        ...
        assert (
            len([m for m in matches if m.severity == "high"]) == 0
