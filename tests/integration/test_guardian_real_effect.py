...
from guardian.harm_detector import analyze_file_operation
...
class GuardedAgent:
    ...
    def execute_with_guardian(
        self, action: AgentAction, user_confirms: bool = False
    ) -> GuardedResult:
        """
...
class TestGuardianActuallyPreventsHarm:
    ...
    def test_guardian_blocks_secret_in_file_write(self):
        ...
        action = AgentAction(
            action_type="file_write", target="config.py", content=secret_content
        )
    ...
    def test_guardian_allows_safe_operations(self):
        ...
        safe_actions = [
            ...
            AgentAction(
                action_type="file_write", target="README.md", content="# Hello"
            ),
        ]
...
class TestGuardianPauseRequiresConfirmation:
    ...
    def test_sensitive_file_requires_confirmation(self):
        ...
        agent = GuardedAgent()
        action = AgentAction(
            action_type="file_write", target=".env", content="DEBUG=true"
        )

        # Without confirmation
        result = agent.execute_with_guardian(action, user_confirms=False)
        assert (
            not result.action_taken
        ), ".env write should NOT proceed without confirmation"
    ...
    def test_cursorrules_modification_requires_confirmation(self):
        ...
        agent = GuardedAgent()
        action = AgentAction(
            action_type="file_write", target=".agentrules", content="# Modified rules"
        )
...
class TestGuardianWithRealFiles:
    ...
    def test_would_have_deleted_critical_file(self):
        ...
        assert (
            "/etc" not in os.listdir("/") if os.name != "nt" else True
        )  # Still exists
    ...
    def test_secret_file_creation_blocked(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            action = AgentAction(
                action_type="file_write", target=secret_file, content=secret_content
            )
    ...
    def test_safe_file_creation_allowed(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            action = AgentAction(
                action_type="file_write", target=safe_file, content=safe_content
            )
...
class TestGuardianStatistics:
    ...
    def test_guardian_blocking_rate(self):
        ...
        harmful_actions = [
            ...
            AgentAction(
                "file_write", "config.py", 'KEY="sk-1234567890abcdefghijklmnop"'
            ),
...
class TestGuardianEndToEnd:
    ...
    def test_developer_workflow_with_guardian(self):
        ...
        workflow = [
            ...
            (
                AgentAction("file_write", "src/app.py", "class App: pass"),
                True,
                "Write code",
            ),
            ...
            (
                AgentAction("command", "rm -rf node_modules/"),
                True,
                "Clean node_modules",
            ),  # Safe-ish
