...
_script_path = (
    Path(__file__).parent.parent.parent / "scripts" / "git" / "install_hooks.py"
)
...
class TestMainEntry:
    ...
    def test_main_calls_install_hooks(self):
        ...
        with patch.object(
            hooks_module, "install_hook", return_value=True
        ) as mock_install:
            with patch.object(sys, "exit") as mock_exit:
                # Simulate running as main
                exec(
                    compile(
                        "if __name__ == '__main__': sys.exit(main())", "<test>", "exec"
                    ),
