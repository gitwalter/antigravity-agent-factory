...
class TestIntegration:
    ...
    def factory_root(self):
        ...
        # File is at tests/unit/sync/test_sync_test_counts.py
        # Root is 4 levels up
        return Path(__file__).parent.parent.parent.parent

    def test_testing_md_exists(self, factory_root):
        """docs/testing/testing.md should exist."""
        testing_md = factory_root / "docs" / "testing" / "testing.md"
        assert testing_md.exists(), "docs/testing/testing.md not found"
    ...
    def test_testing_md_has_expected_structure(self, factory_root):
        ...
        assert (
            total_pattern is not None
        ), "Missing total count pattern (expected 'consists of...** followed by number)"
        assert (
            "tests**" in content or "tests*** " in content
        ), "Missing 'tests**' in total count"
    ...
    def test_counts_are_currently_synced(self, factory_root):
        ...
        if len(changes) > 0:
            # Check if it's just a small difference (likely local vs CI/baseline)
            diff = abs(actual.total - extract_documented_counts(content).total)
            if diff <= 5:
                pytest.skip(
                    f"Test count mismatch is minor ({diff} tests) - likely local environment difference"
                )
