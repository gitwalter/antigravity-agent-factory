...
class TestGetFileVersionGeneric:
    ...
    def test_extracts_version_with_valid_pattern(self, tmp_path):
        ...
        version = get_file_version_generic(
            test_file, r"\*Antigravity Agent Factory v(\d+\.\d+\.\d+)\*"
        )
        assert version == "3.4.0"
    ...
    def test_returns_none_for_missing_file(self, tmp_path):
        ...
        version = get_file_version_generic(missing_file, r"v(\d+\.\d+\.\d+)")
    ...
    def test_returns_none_for_no_match(self, tmp_path):
        ...
        version = get_file_version_generic(test_file, r"v(\d+\.\d+\.\d+)")
        assert version is None
...
class TestSyncFileVersion:
    ...
    def test_sync_updates_file(self, tmp_path):
        ...
        assert result is True
        assert "*Antigravity Agent Factory v2.0.0*" in test_file.read_text(
            encoding="utf-8"
        )
    ...
    def test_returns_false_for_missing_file(self, tmp_path):
        ...
        result = sync_file_version(
            missing_file, r"v[\d.]+", lambda v: f"v{v}", "2.0.0", dry_run=True
        )
    ...
    def test_returns_false_for_no_pattern_match(self, tmp_path):
        ...
        result = sync_file_version(
            test_file, r"v[\d.]+", lambda v: f"v{v}", "2.0.0", dry_run=True
        )
...
class TestVersionLocations:
    ...
    def test_all_locations_have_required_keys(self):
        ...
        for loc in VERSION_LOCATIONS:
            ...
            assert (
                not missing
            ), f"Location {loc.get('name', 'unknown')} missing keys: {missing}"
    ...
    def test_replacement_functions_are_callable(self):
        ...
        for loc in VERSION_LOCATIONS:
            assert callable(
                loc["replacement"]
            ), f"Replacement in {loc['name']} is not callable"

    def test_replacement_functions_produce_valid_strings(self):
        """Replacement functions should return strings with version."""
        test_version = "9.9.9"
        for loc in VERSION_LOCATIONS:
            # Replacement is a lambda that takes one argument: target_version
            result = loc["replacement"](test_version)
            assert isinstance(
                result, str
            ), f"Replacement in {loc['name']} didn't return string"
            assert (
                test_version in result
            ), f"Replacement in {loc['name']} doesn't include version"
...
class TestSyncManifest:
    ...
    def test_detects_out_of_sync_factory_version(self, tmp_path, monkeypatch):
        ...
        manifest.write_text(
            json.dumps({"factory_version": "3.4.0", "files": {}}), encoding="utf-8"
        )
...
class TestIntegration:
    ...
    def factory_root(self):
        ...
        # File is at tests/unit/sync/test_sync_manifest_versions.py
        # Root is 4 levels up
    ...
    def test_changelog_exists_and_has_version(self, factory_root):
        ...
        try:
            ...
            assert (
                version != "0.0.0"
            ), f"CHANGELOG.md should have a version (root: {factory_root})"
            assert re.match(
                r"\d+\.\d+\.\d+", version
    ...
    def test_version_locations_files_exist(self, factory_root):
        ...
        for loc in VERSION_LOCATIONS:
            ...
            assert filepath.exists(), f"File {loc['file']} not found (expected by {loc['name']}, root: {factory_root})"
    ...
    def test_current_state_is_synced(self, factory_root):
        ...
        try:
            ...
            assert (
                all_synced
            ), f"Factory versions out of sync: {changes} (root: {factory_root})"
        finally:
            os.chdir(original_cwd)
