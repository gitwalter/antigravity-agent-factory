...
class TestChangelogGeneratorInit:
    ...
    def test_init_creates_directory(self, tmp_changelog_dir):
        """Test that initialization creates changelog directory."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        assert generator.changelog_dir.exists()
        assert generator.changelog_dir == tmp_changelog_dir
    
    def test_init_existing_directory(self, tmp_changelog_dir):
...
class TestChangelogGeneratorPaths:
    """Tests for changelog path methods."""
    
    def test_get_changelog_path(self, tmp_changelog_dir):
        """Test getting changelog path for a knowledge file."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        path = generator.get_changelog_path("test-patterns.json")
        
        assert path.name == "test-patterns-changelog.json"
        assert path.parent == tmp_changelog_dir
    
    def test_get_changelog_path_with_path_object(self, tmp_changelog_dir):
        """Test getting changelog path with Path object."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        path = generator.get_changelog_path(Path("test-patterns.json"))
        
        assert path.name == "test-patterns-changelog.json"


class TestChangelogGeneratorLoadSave:
    """Tests for loading and saving changelogs."""
    
    def test_load_nonexistent_changelog(self, tmp_changelog_dir):
        """Test loading a changelog that doesn't exist."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        changelog = generator.load_changelog("test-patterns.json")
        
        assert changelog["knowledge_file"] == "test-patterns.json"
        assert changelog["current_version"] == "1.0.0"
        assert changelog["entries"] == []
    
    def test_load_existing_changelog(self, tmp_changelog_dir):
        """Test loading an existing changelog."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        # Create a changelog file
        changelog_path = generator.get_changelog_path("test-patterns.json")
        changelog_data = {
            "knowledge_file": "test-patterns.json",
            "current_version": "1.5.0",
            "entries": [
                {"version": "1.5.0", "date": "2024-01-01", "changes": {}}
            ]
        }
        changelog_path.write_text(json.dumps(changelog_data), encoding="utf-8")
        
        loaded = generator.load_changelog("test-patterns.json")
        
        assert loaded["current_version"] == "1.5.0"
        assert len(loaded["entries"]) == 1
    
    def test_save_changelog(self, tmp_changelog_dir):
        ...
        generator.save_changelog("test-patterns.json", changelog)
        
        changelog_path = generator.get_changelog_path("test-patterns.json")
        assert changelog_path.exists()
        
        loaded = json.loads(changelog_path.read_text(encoding="utf-8"))
        assert loaded["current_version"] == "1.0.0"


class TestChangelogGeneratorCreateEntry:
    """Tests for creating changelog entries."""
    
    def test_create_entry_from_update_result(self, tmp_changelog_dir):
        """Test creating entry from UpdateResult."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        # Create a mock UpdateResult
        update_result = Mock()
        update_result.new_version = "1.1.0"
        update_result.operations = [
            Mock(operation_type="add", path="patterns[1]", new_value={"id": "p2"}),
            Mock(operation_type="modify", path="version", new_value="1.1.0"),
        ]
        
        entry = generator.create_entry_from_update(update_result)
        
        assert entry.version == "1.1.0"
        assert "added" in entry.changes
        assert "changed" in entry.changes
    
    def test_create_entry_from_knowledge_update(self, tmp_changelog_dir, sample_knowledge_update):
        """Test creating entry from KnowledgeUpdate."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        entry = generator.create_entry_from_knowledge_update(sample_knowledge_update)
        
        assert entry.version == sample_knowledge_update.new_version
        assert len(entry.sources) > 0
        assert entry.breaking == sample_knowledge_update.breaking
    
    def test_create_entry_removes_empty_categories(self, tmp_changelog_dir):
        ...
        update_result.new_version = "1.1.0"
        update_result.operations = [
            Mock(operation_type="add", path="new_field", new_value="value"),
        ]
        
        entry = generator.create_entry_from_update(update_result)
        
        # Should only have "added", not empty categories
        assert "added" in entry.changes
        assert len(entry.changes) == 1


class TestChangelogGeneratorAppend:
    """Tests for appending entries."""
    
    def test_append_entry_new_changelog(self, tmp_changelog_dir):
        ...
        generator.append_entry("test-patterns.json", entry)
        
        changelog = generator.load_changelog("test-patterns.json")
        assert changelog["current_version"] == "1.1.0"
        assert len(changelog["entries"]) == 1
    
    def test_append_entry_existing_changelog(self, tmp_changelog_dir):
        ...
        generator.append_entry("test-patterns.json", entry1)
        ...
        generator.append_entry("test-patterns.json", entry2)
        
        changelog = generator.load_changelog("test-patterns.json")
        assert changelog["current_version"] == "1.2.0"
        assert len(changelog["entries"]) == 2
        # Newest should be first
        assert changelog["entries"][0]["version"] == "1.2.0"
    
    def test_append_entry_with_migration_notes(self, tmp_changelog_dir):
        ...
        entry = ChangelogEntry(
            ...
            breaking=True,
            migration_notes="Update your code to use new API",
        )
        
        generator.append_entry("test-patterns.json", entry)
        
        changelog = generator.load_changelog("test-patterns.json")
        assert "migration" in changelog["entries"][0]
        assert changelog["entries"][0]["migration"]["instructions"] == "Update your code to use new API"


class TestChangelogGeneratorMarkdown:
    """Tests for markdown generation."""
    
    def test_generate_markdown_empty(self, tmp_changelog_dir):
        """Test generating markdown for empty changelog."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        markdown = generator.generate_markdown("test-patterns.json")
        
        assert "# Changelog for test-patterns.json" in markdown
        assert "Current Version: **1.0.0**" in markdown
    
    def test_generate_markdown_with_entries(self, tmp_changelog_dir):
        ...
        assert "*Sources: github*" in markdown
    
    def test_generate_markdown_with_max_entries(self, tmp_changelog_dir):
        ...
        for i in range(5):
            entry = ChangelogEntry(
                ...
                changes={"added": [f"Feature {i}"]},
            )
            generator.append_entry("test-patterns.json", entry)
        
        markdown = generator.generate_markdown("test-patterns.json", max_entries=2)
        
        # Should only show 2 entries
        assert markdown.count("## [") == 2
    
    def test_generate_markdown_breaking_changes(self, tmp_changelog_dir):
...
class TestChangelogGeneratorSummary:
    """Tests for summary generation."""
    
    def test_generate_summary_no_files(self, tmp_changelog_dir):
        """Test generating summary with no changelog files."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        summary = generator.generate_summary()
        
        assert "Knowledge Evolution Summary" in summary
        assert "Total files tracked: 0" in summary
    
    def test_generate_summary_with_files(self, tmp_changelog_dir):
        """Test generating summary with changelog files."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        # Create changelogs for multiple files
        for file_name in ["file1.json", "file2.json"]:
            ...
            generator.append_entry(file_name, entry)
        
        summary = generator.generate_summary()
        
        assert "Total files tracked: 2" in summary
        assert "file1.json" in summary
        assert "file2.json" in summary
    
    def test_generate_summary_with_filter(self, tmp_changelog_dir):
        ...
        summary = generator.generate_summary(["file1.json"])
        
        assert "file1.json" in summary
        assert "file2.json" not in summary


class TestChangelogGeneratorVersionDiff:
    """Tests for version diff functionality."""
    
    def test_get_version_diff(self, tmp_changelog_dir):
        """Test getting changes between versions."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        # Add multiple version entries
        versions = ["1.0.0", "1.1.0", "1.2.0", "1.3.0"]
        for version in versions:
            entry = ChangelogEntry(
                version=version,
                date="2024-01-01",
                changes={"added": [f"Feature {version}"]},
            )
            generator.append_entry("test-patterns.json", entry)
        
        diff = generator.get_version_diff("test-patterns.json", "1.0.0", "1.2.0")
        
        # Should include 1.2.0 and 1.1.0 (between 1.0.0 and 1.2.0)
        assert len(diff) == 2
        assert diff[0].version == "1.2.0"
        assert diff[1].version == "1.1.0"
    
    def test_get_version_diff_no_matches(self, tmp_changelog_dir):
        """Test version diff with no matching versions."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        diff = generator.get_version_diff("test-patterns.json", "1.0.0", "2.0.0")
...
class TestIntegration:
    ...
    async def test_full_update_workflow(self, tmp_knowledge_dir, tmp_changelog_dir, sample_knowledge_update, mock_config_manager):
        ...
        # Setup
        engine = UpdateEngine(tmp_knowledge_dir)
        changelog_gen = ChangelogGenerator(tmp_changelog_dir)
        ...
        # Generate changelog
        entry = changelog_gen.create_entry_from_knowledge_update(sample_knowledge_update)
        changelog_gen.append_entry(sample_knowledge_update.target_file, entry)
        ...
        # Verify changelog
        markdown = changelog_gen.generate_markdown(sample_knowledge_update.target_file)
        assert sample_knowledge_update.new_version in markdown
...
class TestChangelogGeneratorEdgeCases:
    """Additional edge case tests for ChangelogGenerator."""
    
    def test_create_entry_empty_operations(self, tmp_changelog_dir):
    ...
    def test_create_entry_unknown_operation_type(self, tmp_changelog_dir):
        ...
        update_result.new_version = "1.1.0"
        update_result.operations = [
            Mock(operation_type="unknown", path="test", new_value=None),
        ]
        
        entry = generator.create_entry_from_update(update_result)
        
        # Should default to "changed" category
        assert "changed" in entry.changes
    
    def test_generate_markdown_empty_changes(self, tmp_changelog_dir):
        ...
        generator.append_entry("test.json", entry)
        
        markdown = generator.generate_markdown("test.json")
        
        assert "[1.0.0]" in markdown
        # Should not have any change categories
    
    def test_get_changelog_path_nested(self, tmp_changelog_dir):
        """Test changelog path with nested file path."""
        generator = ChangelogGenerator(tmp_changelog_dir)
        
        path = generator.get_changelog_path("subdir/test-patterns.json")
