...
def temp_repo():
    ...
    (root / "README.md").write_text("[Target](docs/target.md) and [Missing](missing.md)", encoding="utf-8")
...
async def test_link_normalization(temp_repo):
    ...
    checker = LinkChecker(temp_repo, fix_mode=True, check_external=False, normalize=True)
    checker.build_index()
    ...
    assert "(/docs/target.md)" not in content # Specific check for the markdown link parenthesized part
