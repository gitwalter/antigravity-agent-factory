...
class TestKnowledgeFileStructure:
    ...
    def test_has_axiom_alignment(self, research_first_pattern: dict):
        ...
        assert any(
            "humility" in key.lower() or "A2" in key for key in alignment.keys()
        ), "Should align with humility axiom"
        assert any(
            "transparency" in key.lower() or "A3" in key for key in alignment.keys()
...
class TestWorkflowSteps:
    ...
    def test_workflow_includes_build_step(self, research_first_pattern: dict):
        ...
        build_steps = [
            ...
            if "build" in k.lower() or "implement" in k.lower()
        ]
        assert len(build_steps) > 0, "Workflow should include build step"

    def test_workflow_order_is_research_before_build(
        self, research_first_pattern: dict
    ):
        ...
        research_pos = next(
            (i for i, k in enumerate(keys) if "research" in k.lower()), -1
        )
...
class TestTriggerDefinitions:
    ...
    def test_has_trigger_definitions(self, research_first_pattern: dict):
        ...
        assert "triggers" in research_first_pattern or any(
            "trigger" in str(v).lower() for v in research_first_pattern.values()
        )

    def test_triggers_include_performance_optimization(
        self, research_first_pattern: dict
    ):
    ...
    def test_has_counter_indicators(self, research_first_pattern: dict):
        ...
        assert (
            has_skip or has_counter or has_counter_in_workflow
...
class TestBenefitsDocumentation:
    ...
    def test_documents_multiplied_value(self, research_first_pattern: dict):
        ...
        assert (
            "multiplied" in pattern_str or "reusable" in pattern_str
...
class TestAntiPatterns:
    ...
    def test_warns_against_not_invented_here(self, research_first_pattern: dict):
        ...
        assert (
            "invented" in anti_pattern_text or "dismiss" in anti_pattern_text
    ...
    def test_warns_against_analysis_paralysis(self, research_first_pattern: dict):
        ...
        assert (
            "paralysis" in anti_pattern_text or "over-research" in anti_pattern_text
        ), "Should warn against analysis paralysis"
...
class TestExamples:
    ...
    def test_reactive_indexing_example(self, research_first_pattern: dict):
        ...
        assert (
            "reactive" in example_str or "indexing" in example_str
        ), "Should include reactive indexing as an example"
