...
class TestPyPIAdapterMocked:
    ...
    async def test_get_package_info_success(self, adapter, mock_pypi_response):
        ...
        with patch.object(
            adapter, "_get_session", new=AsyncMock(return_value=mock_session)
        ):
    ...
    async def test_validate_connection_success(self, adapter, mock_pypi_response):
        ...
        with patch.object(
            adapter, "_get_package_info", return_value=mock_pypi_response
        ):
    ...
    async def test_fetch_updates_with_target_files(self, adapter, mock_pypi_response):
        ...
        with patch.object(
            ...
            with patch.object(adapter, "_close_session", return_value=None):
                updates = await adapter.fetch_updates(
                    target_files=["fastapi-patterns.json"]
                )
    ...
    async def test_fetch_updates_with_since_filter(self, adapter, mock_pypi_response):
        ...
        mock_pypi_response["releases"]["0.115.0"][0]["upload_time"] = (
            old_date.isoformat()
        )
    ...
    def test_analyze_version_change_deprecation(self, adapter):
        ...
        package = TrackedPackage(name="fastapi", knowledge_file="fastapi-patterns.json")
        info = {
            "description": "This version contains breaking changes and deprecated features"
        }
    ...
    async def test_check_package_no_version(self, adapter):
        ...
        mock_response = {"info": {"name": "test-package"}, "releases": {}}
    ...
    async def test_check_package_no_upload_time(self, adapter, mock_pypi_response):
        ...
        with patch.object(
            ...
            package = TrackedPackage(
                name="fastapi", knowledge_file="fastapi-patterns.json"
            )
            updates = await adapter._check_package(package)
...
class TestNPMAdapterMocked:
    ...
    def config(self):
        """Create adapter config for testing."""
        return AdapterConfig(enabled=True, timeout_seconds=30, api_key="test-token")
    ...
    def mock_npm_response(self):
        ...
        return {
            ...
            "versions": {
                "18.2.0": {"version": "18.2.0", "engines": {"node": ">=16.0.0"}}
            },
            "time": {"18.2.0": "2024-01-15T10:30:00.000Z"},
        }
    ...
    async def test_fetch_updates_with_since_filter(self, adapter, mock_npm_response):
        ...
        old_date = datetime.now(timezone.utc) - timedelta(days=30)
        mock_npm_response["time"]["18.2.0"] = old_date.isoformat().replace(
            "+00:00", "Z"
        )
...
class TestGitHubAdapterMocked:
    ...
    def config(self):
        """Create adapter config for testing."""
        return AdapterConfig(enabled=True, api_key="ghp_test_token", timeout_seconds=30)
    ...
    async def test_make_request_success(self, adapter, mock_releases_response):
        ...
        mock_response.headers.get = MagicMock(
            side_effect=lambda key, default=None: headers_dict.get(key, default)
        )
        mock_response.__aenter__ = AsyncMock(return_value=mock_response)
        mock_response.__aexit__ = AsyncMock(return_value=None)
    ...
    async def test_make_request_rate_limited(self, adapter):
        ...
        mock_response.headers = MagicMock()
        mock_response.headers.get = MagicMock(
            side_effect=lambda key, default=None: {"X-RateLimit-Remaining": "0"}.get(
                key, default
            )
        )
        mock_response.__aenter__ = AsyncMock(return_value=mock_response)
    ...
    async def test_validate_connection_success(self, adapter, mock_rate_limit_response):
        """Test successful connection validation."""
        with patch.object(
            adapter, "_make_request", return_value=mock_rate_limit_response
        ):
            result = await adapter.validate_connection()
            assert result is True
    ...
    async def test_fetch_updates_success(self, adapter, mock_releases_response):
        ...
        repo = TrackedRepository(
            owner="tiangolo", name="fastapi", knowledge_file="fastapi-patterns.json"
        )

        with patch.object(
            adapter, "_make_request", return_value=mock_releases_response
        ):
            with patch.object(adapter, "_close_session", return_value=None):
                updates = await adapter.fetch_updates()
    ...
    @pytest.mark.asyncio
    async def test_check_releases_with_since_filter(
        self, adapter, mock_releases_response
    ):
        ...
        # Set old release date (make it timezone-aware)
        old_date = datetime.now(timezone.utc) - timedelta(days=30)
        mock_releases_response[0]["published_at"] = old_date.isoformat().replace(
            "+00:00", "Z"
        )
    ...
    def test_parse_release_notes_added(self, adapter):
        ...
        repo = TrackedRepository(owner="test", name="repo", knowledge_file="test.json")
        notes = "### Added\n- New feature 1\n- New feature 2"
    ...
    def test_is_breaking(self, adapter):
        ...
        release = {"body": "Bug fixes and improvements", "tag_name": "v1.1.0"}
    ...
    async def test_get_readme(self, adapter):
        ...
        mock_readme_response = {"content": encoded_content, "encoding": "base64"}
    ...
    def test_determine_priority_major_version_tag(
        self, adapter, mock_releases_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_updates_with_target_files(
        self, adapter, mock_releases_response
    ):
        """Test fetch_updates with target file filtering."""
        with patch.object(
...
class TestCommunityAdapterMocked:
    ...
    async def test_check_source_no_pushed_at(self, adapter):
        ...
        mock_repo_info = {"id": 12345, "name": "test-repo", "stargazers_count": 100}
...
class TestDocsAdapterMocked:
    ...
    @pytest.mark.asyncio
    async def test_get_framework_version_pypi(
        self, adapter, mock_pypi_version_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_updates_with_since_filter(
        self, adapter, mock_pypi_version_response
    ):
        """Test fetch_updates with date filtering."""
...
class TestFeedbackAdapterMocked:
    ...
    def sample_feedback_data(self):
        ...
        return {
            ...
            "suggestions": ["Add more examples", "Improve error handling"],
    ...
    async def test_fetch_updates_success(
        self, adapter, sample_feedback_data, temp_feedback_dir
    ):
        """Test successful fetch_updates."""
    ...
    def test_analyze_feedback_common_issues(self, adapter):
        ...
        feedback_items = [
            ...
            ProjectFeedback(
                project_id="proj2",
                blueprint_used="test",
                issues=[{"type": "dependency_conflict", "description": "Conflict"}],
                knowledge_files_used=["test.json"],
            ),
            ProjectFeedback(
                project_id="proj3",
    ...
    def test_patterns_to_changes(self, adapter):
        ...
        patterns = [
            ...
            {"type": "suggestion", "content": "Add more examples", "frequency": 2},
        ]
    ...
    def test_patterns_to_changes_unknown_type(self, adapter):
        ...
        patterns = [{"type": "unknown_type", "data": "test"}]
    ...
    @pytest.mark.asyncio
    async def test_fetch_updates_multiple_feedback_files(
        self, adapter, temp_feedback_dir
    ):
        ...
        for i in range(3):
            feedback_data = {
                ...
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "success_metrics": {},
                "issues": [{"type": "common_issue", "description": "Common issue"}],
                "suggestions": [],
                "knowledge_files_used": ["test.json"],
            }
    ...
    def test_analyze_feedback_suggestion_threshold(self, adapter):
        ...
        if "test.json" in analysis:
            suggestions = [
                p for p in analysis["test.json"] if p["type"] == "suggestion"
            ]
    ...
    def test_record_feedback_overwrites_existing(
        self, adapter, temp_feedback_dir, sample_feedback_data
    ):
...
class TestBaseAdapterCaching:
    ...
    def test_cache_expiration(self, adapter):
        ...
        adapter._cache["test_key"]["timestamp"] = datetime.now(
            timezone.utc
        ) - timedelta(hours=2)
