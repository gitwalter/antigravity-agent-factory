...
class TestScanContent:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                'AWS_KEY = "AKIAIOSFODNN7A1B2C3D"',
                "AWS Access Key ID",
            ),  # Valid format, no "EXAMPLE"
            (
                'token = "ghp_1234567890abcdefghijklmnopqrstuvwxyz"',
                "GitHub Personal Access Token",
            ),
            (
                'GITLAB_TOKEN = "glpat-Abc123Def456Ghi789Jkl"',
                "GitLab Personal Access Token",
            ),  # Realistic
            (
                'SENDGRID_KEY = "SG.1234567890abcdefghijkl.12345678901234567890123456789012345678901234"',
                "SendGrid API Key",
            ),
            (
                'GOOGLE_API = "AIzaSyC1234567890abcdefghijk"',
                "Google API Key",
            ),  # 27 chars after AIza
        ],
    )
    def test_high_severity_api_keys(self, content, secret_type):
        ...
        assert any(
            m.severity == "high" for m in matches
        ), f"{secret_type} should be high severity"
    ...
    def test_database_connection_strings(self, content, db_type):
        ...
        assert any(
            m.severity == "high" for m in matches
        ), f"{db_type} should be high severity"
