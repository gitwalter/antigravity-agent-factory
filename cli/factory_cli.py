...
def run_quickstart(output_dir: str = None, blueprint_id: str = None) -> None:
    ...
    try:
        ...
        display_error_with_help(
            str(e), "Make sure you have write permissions to the output directory."
        )
        sys.exit(1)
...
def interactive_mode(output_dir: str) -> None:
    ...
    primary_language = (
        input("1. Primary programming language [python]: ").strip() or "python"
    )

    frameworks_input = input(
        "2. Frameworks (comma-separated, e.g., fastapi,sqlalchemy): "
    ).strip()
    frameworks = (
        [f.strip() for f in frameworks_input.split(",")] if frameworks_input else []
    )
    ...
    elif primary_language == "typescript" and "react" in [
    ...
    triggers_input = (
        input("1. Workflow triggers (comma-separated) [manual]: ").strip() or "manual"
    )
    ...
    agents_input = (
        input("1. Agents to include (comma-separated) [code-reviewer]: ").strip()
        or "code-reviewer"
    )
    agents = [a.strip() for a in agents_input.split(",")]

    print(
        "   Available skills: bugfix-workflow, feature-workflow, grounding, tdd, code-templates"
    )
    skills_input = (
        input("2. Skills to include (comma-separated) [bugfix-workflow,tdd]: ").strip()
        or "bugfix-workflow,tdd"
    )
    ...
    if enable_pm == "y":
        ...
        pm_doc_backend = (
            input("3. Documentation backend [github-wiki]: ").strip() or "github-wiki"
        )
    ...
    server_configs = {
        ...
        "deepwiki": {
            ...
            "purpose": "GitHub repo docs",
        },
        "huggingface": {
            "name": "huggingface",
            "url": "https://huggingface.co/mcp",
            "env": {"HF_TOKEN": "${HF_TOKEN}"},
            "purpose": "HuggingFace Hub",
        },
        "mlflow": {
            "name": "mlflow",
        ...
        "langgraph": {
            "name": "langgraph",
            "purpose": "Agent orchestration (requires deployment URL)",
        },
        "knowledge-graph": {
        ...
        "sap-abap-documentation": {
            "name": "sap-abap-documentation",
            "url": "https://mcp-abap.marianzeis.de/mcp",
            "purpose": "ABAP keywords, DSAG guidelines, Clean ABAP",
        },
    }
    ...
    if pack_choice == "1":
        ...
        print(
            "   AI/ML: huggingface, mlflow, langgraph, knowledge-graph, ollama, jupyter"
        )
    ...
    if pm_enabled:
        agents.extend(
            ["product-owner", "sprint-master", "task-manager", "reporting-agent"]
        )
        skills.extend(
            [
                "create-epic",
...
def generate_from_blueprint(
    ...
    """
    blueprint_path = (
        get_factory_root() / ".agent" / "blueprints" / blueprint_id / "blueprint.json"
    )
        ...
        if blueprint_id:
            ...
            if blueprint_path.exists():
                ...
                config = ProjectConfig(
                    ...
                    project_description=blueprint.get("metadata", {}).get(
                        "description", ""
                    ),
...
def _check_blueprint_knowledge(blueprint_id: str) -> None:
    ...
    try:
        blueprint_path = (
            get_factory_root() / "blueprints" / blueprint_id / "blueprint.json"
        )
...
def _offer_knowledge_extension(blueprint_id: str = None) -> None:
    ...
    try:
        from scripts.analysis.knowledge_gap_analyzer import KnowledgeGapAnalyzer

        factory_root = get_factory_root()
        analyzer = KnowledgeGapAnalyzer(
            factory_root / "knowledge", factory_root / "scripts" / "taxonomy"
        )
...
def _create_default_config(repo_path: str, inventory) -> ProjectConfig:
    ...
    return ProjectConfig(
        ...
        domain="general",
        primary_language=inventory.tech_stack.languages[0]
        if inventory.tech_stack.languages
        else "python",
...
def _interactive_conflict_resolver(prompt):
    ...
    Returns:
        ConflictResolution: User's chosen resolution.
    """
    from scripts.git.merge_strategy import ConflictResolution  # noqa: F401
...
def rollback_session(repo_path: str, session_id: str = None) -> None:
    ...
    """
    from scripts.git.backup_manager import BackupManager
    ...
    if not session_id:
        ...
        for i, session in enumerate(sessions, 1):
            status = (
                ...
                else ("completed" if session.completed else "incomplete")
            )
        ...
        try:
            choice = input(
                "Enter session number to rollback (or 'q' to quit): "
            ).strip()
            if choice.lower() == "q":
                return
    ...
    confirm = (
        input(
            f"\nRollback session {session_id}? This will restore original files. [y/N]: "
        )
        .strip()
        .lower()
    )
...
def analyze_knowledge_gaps(scope: str = "all", filter_value: str = None) -> None:
    ...
    """
    from scripts.analysis.knowledge_gap_analyzer import (
    ...
    try:
        ...
        if scope == "topic" and filter_value:
            result.gaps = [
                g for g in result.gaps if filter_value.lower() in g.topic.name.lower()
            ]
        elif scope == "domain" and filter_value:
            result.gaps = [
                g
                for g in result.gaps
                if filter_value.lower() in g.coverage.topic_path.lower()
            ]
        ...
        print("  python cli/factory_cli.py --extend-knowledge <topic_name>")
...
def suggest_extensions(blueprint_id: str) -> None:
    ...
    try:
        ...
        print(
            f"Found {len(suggestions)} knowledge files that could enhance this blueprint:"
        )
        print()
...
def extend_knowledge_topic(
    topic_name: str, auto_apply: bool = False, use_mock_llm: bool = False
) -> None:
...
def validate_dependencies() -> int:
    """Validate the Factory dependency graph.
    ...
    print()
    print("=" * 60)
    print("  Factory Dependency Graph Validation")
    print("=" * 60)
    print()

    factory_root = Path(__file__).parent.parent
    validator = DependencyValidator(factory_root)
    ...
    if result.broken_refs:
        print("[ERROR] Broken references (hard dependencies):")
    ...
    if result.is_valid:
        ...
        print("[OK] Dependency graph is valid")
...
def show_dependents(node_id: str) -> None:
    """Show what depends on a given node.

    Args:
        node_id: Node identifier (e.g., knowledge:fastapi-patterns.json)
    """
    from scripts.validation.dependency_validator import DependencyValidator

    print()
    print("=" * 60)
    print(f"  Dependents of: {node_id}")
    print("=" * 60)
    print()
    ...
    dependents = validator.reverse_lookup(node_id)
    ...
    if dependents:
        ...
        print("No nodes directly depend on this artifact.")
    print()


def show_impact_analysis(node_id: str) -> None:
    """Show impact analysis for a node (all transitively affected).

    Args:
        node_id: Node identifier (e.g., skill:grounding)
    """
    from scripts.validation.dependency_validator import DependencyValidator
    ...
    affected = validator.impact_analysis(node_id)

    if affected:
        print(f"Changes to {node_id} would affect {len(affected)} artifacts:")
        print()
        ...
        for node_type, nodes in sorted(by_type.items()):
            print(f"  {node_type.upper()} ({len(nodes)}):")
            for node in sorted(nodes):
                print(f"    - {node}")
            print()
    else:
        print("No nodes would be affected by changes to this artifact.")
    print()


def show_dependency_order() -> None:
    """Show topological installation order for all artifacts."""
    from scripts.validation.dependency_validator import DependencyValidator
    from graphlib import CycleError

    print()
    print("=" * 60)
    print("  Dependency Installation Order")
    print("=" * 60)
    print()
    ...
    try:
        ...
        print(f"Install order ({len(order)} artifacts):")
        print()
        ...
        print("        Fix circular dependencies first.")
        print()


def show_dependency_stats() -> None:
...
def export_dependency_graph(output_file: str) -> None:
    ...
    print(f"[OK] Exported dependency graph to {output_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Antigravity Agent Factory - Generate Antigravity agent development systems",
...
Examples:
  ...
  %(prog)s --validate-deps                                    # Full validation
  %(prog)s --dependents knowledge:fastapi-patterns.json       # Who depends on this?
  %(prog)s --impact skill:grounding                           # Impact analysis
  %(prog)s --dep-order                                        # Installation order
  %(prog)s --dep-stats                                        # Graph statistics
  %(prog)s --export-deps deps.json                            # Export to JSON
  ...
  %(prog)s --extend-knowledge constitutional_ai              # Show extension instructions
    ...
    parser.add_argument(
        "--list-blueprints", action="store_true", help="List all available blueprints"
    )
    ...
    parser.add_argument(
        "--version", action="version", version="Antigravity Agent Factory 2.0.0"
    )
    ...
    parser.add_argument(
        "--rollback", type=str, metavar="REPO", help="Rollback an onboarding session"
    )
    ...
    parser.add_argument(
        "--validate-deps",
        action="store_true",
        help="Validate the Factory dependency graph (cycles, broken refs)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="NODE",
        help="Show what depends on a node (e.g., knowledge:fastapi-patterns.json)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="NODE",
        help="Show impact analysis for a node (all transitively affected)",
    )

    parser.add_argument(
        "--dep-order",
        action="store_true",
        help="Show topological installation order for all artifacts",
    )

    parser.add_argument(
        "--dep-stats", action="store_true", help="Show dependency graph statistics"
    )
    ...
    # Handle dependency validation commands
    if args.validate_deps:
        exit_code = validate_dependencies()
        sys.exit(exit_code)

    if args.dependents:
        show_dependents(args.dependents)
        return

    if args.impact:
        show_impact_analysis(args.impact)
        return

    if args.dep_order:
        show_dependency_order()
        return
    ...
    if args.export_deps:
        export_dependency_graph(args.export_deps)
        return

    # Handle knowledge extension commands
    if args.analyze_gaps:
        analyze_knowledge_gaps(scope=args.gap_scope, filter_value=args.gap_filter)
        return
