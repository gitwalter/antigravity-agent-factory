...
def create_context(persist_path: Optional[str] = None) -> SocietyContext:
    ...
    return SocietyContext.create_default(
        name="Factory Agent Society", persist_path=persist_path
    )
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    print(
        f"  Messages: {stats.get('messages_sent', 0)} sent, {stats.get('messages_received', 0)} received"
    )
...
def cmd_events(args: argparse.Namespace) -> int:
    ...
    for event in events:
        ...
        print(
            f"{event.sequence:<6} {event.agent.id:<20} {event.action.type.value:<12} {target:<15} {timestamp}"
        )
...
def cmd_agents(args: argparse.Namespace) -> int:
    ...
    if hasattr(args, "agent") and args.agent:
        ...
        print(
            f"  Reputation: {reputation.current_score:.3f} ({reputation.trust_level})"
        )
        ...
        for agent_id in agents:
            ...
            print(
                f"{agent_id:<25} {reputation.current_score:<12.3f} {reputation.trust_level:<15} {event_count}"
            )
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    if result.valid:
        ...
        print("  [FAIL] Chain integrity compromised")
...
def main():
    ...
    parser.add_argument(
        "--persist", "-p", help="Path to persistence directory", default=None
    )
    ...
    events_parser.add_argument(
        "--limit", "-n", type=int, default=20, help="Limit results"
    )
    ...
    violations_parser.add_argument(
        "--limit", "-n", type=int, default=10, help="Limit results"
    )
