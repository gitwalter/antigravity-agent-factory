...
class BundleManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "BundleManifest":
        ...
        return cls(
            ...
            checksums=[
                ComponentChecksum.from_dict(c) for c in data.get("checksums", [])
            ],
...
def sign_manifest(
    manifest: BundleManifest, private_key_hex: str, signer_id: str
) -> BundleManifest:
    """
    ...
    if not CRYPTO_AVAILABLE:
        raise ImportError(
            "Cryptographic signing requires PyNaCl. Install with: pip install pynacl"
        )
...
def create_manifest_from_bundle(bundle: "AgentBundle") -> BundleManifest:
    ...
    checksums = [
        ComponentChecksum(path=c.path, checksum=c.checksum) for c in bundle.components
    ]
...
class CompatibilityRequirements:
    ...
    def check_compatibility(
        ...
        if self._version_lt(target_version, self.min_factory_version):
            missing.append(
                f"Factory version {self.min_factory_version} required (have {target_version})"
            )
