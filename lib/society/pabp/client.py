...
from lib.society.pabp.adapters import PlatformAdapter, detect_platform
...
class PABPClient:
    ...
    def __init__(
        self, project_root: Path, target_adapter: Optional[PlatformAdapter] = None
    ):
        self.project_root = project_root
        self.target_adapter = target_adapter or detect_platform(project_root)

    def pull_updates(
        self, source: Union[str, Path], dry_run: bool = False
    ) -> UpdateResult:
        """
    ...
    def _install_from_path(self, source_path: Path, dry_run: bool) -> UpdateResult:
        ...
        if (source_path / "manifest.json").exists() and (
        ...
        for comp_type, name, content_path in components:
            try:
                ...
                # Transform content
                transformed = self.target_adapter.transform_content(
                    content, source_adapter
                )
        ...
        audit_log = self._create_audit_log(
            source_path, added, modified, errors, dry_run
        )
    ...
    def _install_from_bundle(self, source_path: Path, dry_run: bool) -> UpdateResult:
        ...
        for pattern in patterns:
            found_files.extend(
                components_dir.rglob(pattern)
            )  # rglob acts on dir, but pattern should be simpler if doing recursive
        ...
        for pabp_file in components_dir.rglob("*.pabp.json"):
            try:
                ...
                target_path = self._get_target_path(
                    comp_type, safe_name, suggested_path
                )
                full_target_path = self.project_root / target_path
    ...
    def _convert_pabp_component(
        self, data: Dict[str, Any], comp_type: str
    ) -> Optional[str]:
    ...
    def _convert_mcp_config(self, data: Dict[str, Any]) -> str:
        ...
        return data.get("files", {}).get(
            "setup-guide.md", "# MCP Config\nSee files in this directory."
        )

    def _discover_components(
        self, source_path: Path, adapter: PlatformAdapter
    ...
    def _get_target_path(
        self, comp_type: str, name: str, suggested_path: str = ""
    ) -> Path:
    ...
    def _install_rules(
        ...
        if is_target_dir:
            ...
            if source_rules.is_file():
                ...
                translated = self.target_adapter.translate_rules(
                    content, source_adapter
                )
                ...
                if (
                    ...
                    or target_file.read_text(encoding="utf-8") != translated
                ):
                    if not dry_run:
            ...
            if (
                not target_rules.exists()
                or target_rules.read_text(encoding="utf-8") != translated
            ):
                if not dry_run:
