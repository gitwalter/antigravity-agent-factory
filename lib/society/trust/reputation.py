...
class ReputationScore:
    ...
    def add_event(self, event: ReputationEvent) -> None:
        ...
        self.current_score = max(
            self.MIN_SCORE, min(self.MAX_SCORE, self.current_score + event.delta)
        )
        self.last_updated = datetime.now(timezone.utc)

    def get_recent_history(
        self, limit: int = 10, window: Optional[timedelta] = None
...
class ReputationSystem:
    ...
    def record_compliance(
        ...
        event = ReputationEvent(
            ...
            delta=delta,
            reason=reason
            or f"Axiom {'compliance' if compliant else 'violation'}{': ' + axiom if axiom else ''}",
        )
    ...
    def record_contract_event(
        self, agent_id: str, fulfilled: bool, contract_id: str, reason: str = ""
    ) -> None:
        """
        ...
        delta = (
            self.CONTRACT_FULFILLED_BONUS
            if fulfilled
            else self.CONTRACT_VIOLATED_PENALTY
        )
        ...
        event = ReputationEvent(
            ...
            delta=delta,
            reason=reason
            or f"Contract {contract_id} {'fulfilled' if fulfilled else 'violated'}",
            source=contract_id,
        )
    ...
    def record_endorsement(
        self, agent_id: str, endorser_id: str, positive: bool = True, reason: str = ""
    ) -> None:
        """
        ...
        event = ReputationEvent(
            ...
            delta=delta,
            reason=reason
            or f"{'Positive' if positive else 'Negative'} endorsement from {endorser_id}",
            source=endorser_id,
        )
    ...
    def get_trusted_agents(self, min_score: float = 50.0) -> List[str]:
    ...
    def get_rankings(self, limit: int = 10) -> List[tuple]:
        ...
        """
        rankings = [
            (agent_id, score.current_score) for agent_id, score in self._scores.items()
        ]
    ...
    def export(self) -> Dict[str, Any]:
        ...
        return {agent_id: score.to_dict() for agent_id, score in self._scores.items()}
