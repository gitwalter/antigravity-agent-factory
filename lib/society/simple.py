...
class SimpleSociety:
    ...
    def send(
        ...
        if from_agent not in self._bridges:
            return SendResult(verified=False, error=f"Agent '{from_agent}' not registered")
        
        if to_agent not in self._bridges:
            return SendResult(verified=False, error=f"Agent '{to_agent}' not registered")
    ...
    def create_contract(
        ...
        for p in parties:
            ...
            party_list.append(Party(agent_id=agent_id, role=role, public_key=public_key))
        ...
        for o in (obligations or []):
            ...
            obl_dict[agent_id].append(Obligation(trigger="on_request", action=action, parameters=params))
