...
class AxiomComplianceMonitor:
    ...
    def verify(self, event: AgentEvent) -> VerificationResult:
        ...
        if has_violations:
            ...
            if self._should_escalate(event.agent.id):
                escalated = True
                logger.error(f"Escalation threshold reached for agent {event.agent.id}")
    ...
    def _should_escalate(self, agent_id: str) -> bool:
        ...
        recent_violations = [v for v in self._violation_history[agent_id] if v > cutoff]

        return len(recent_violations) >= self.escalation_threshold

    def get_violation_count(
        self, agent_id: str, window: Optional[timedelta] = None
    ) -> int:
        """
        ...
        return len([v for v in self._violation_history[agent_id] if v > cutoff])
