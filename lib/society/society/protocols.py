...
class DirectProtocol(CommunicationProtocol):
    ...
    def send(self, message: Message) -> bool:
        ...
        if not message.recipients:
            logger.warning(
                f"Direct protocol requires recipients for message {message.id}"
            )
            return False
...
class BroadcastProtocol(CommunicationProtocol):
    ...
    def send(self, message: Message) -> bool:
        ...
        logger.debug(
            f"Broadcast message {message.id} to {len(recipients)} subscribers on topic '{topic}'"
        )
...
class MessageRouter:
    ...
    def register_protocol(
        self, protocol: CommunicationProtocol, default: bool = False
    ) -> None:
