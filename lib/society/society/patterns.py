...
class SocietyPattern(ABC):
    ...
    def vote(self, proposal_id: str, agent_id: str, in_favor: bool) -> bool:
        ...
        logger.info(f"Vote cast: {agent_id} voted {'yes' if in_favor else 'no'} on {proposal_id}")
        return True
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "open_proposals": len([p for p in self._proposals.values() if p.status == "open"]),
        }
...
class DAOSociety(SocietyPattern):
    ...
    def evaluate_proposal(self, proposal: Proposal) -> bool:
        ...
        voted_stake = sum(
            ...
            for agent_id in proposal.votes.keys()
        )
