...
class HybridVerificationResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "axiom_violations": [
                r.axiom_id.value for r in self.axiom_result.results if not r.passed
            ]
            if self.axiom_result
            else [],
...
class HybridVerificationSystem:
    ...
    @classmethod
    def create_default(
        cls, config: Optional[SystemConfig] = None, with_blockchain: bool = False
    ...
    def record_event(
        self, event: AgentEvent, level: Optional[VerificationLevel] = None
        ...
        if level.value in ["full", "anchored"]:
            ...
            if result.trust_score < 30.0:
                logger.warning(
                    f"Low trust agent: {event.agent.id} ({result.trust_score})"
                )
    ...
    def add_violation_handler(
        self, handler: Callable[[HybridVerificationResult], None]
    ) -> None:
        """Add a violation handler."""
        self._violation_handlers.append(handler)

    def _update_reputation(self, agent_id: str, positive: bool, reason: str) -> None:
