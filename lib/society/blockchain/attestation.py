...
class AttestationRegistry:
    ...
    def create_attestation(
        ...
        logger.info(
            f"Attestation created: {attestation_id} by {attester} for {subject}"
        )
    ...
    def get_for_subject(self, subject: str) -> List[Attestation]:
        ...
        return [self._attestations[id] for id in ids if id in self._attestations]
    ...
    def verify(self, attestation_id: str, verify_signature_fn=None) -> Dict[str, Any]:
        """
        ...
        if not attestation.is_valid:
            return {
                ...
                "expires": attestation.expires.isoformat()
        ...
        if verify_signature_fn and attestation.signature:
            ...
            if not verify_signature_fn(
                attestation.attester, expected_hash, attestation.signature
            ):
                return {
                    "valid": False,
    ...
    def anchor_attestation(self, attestation_id: str) -> bool:
        ...
        if anchor:
            ...
            if self.anchor_service.submit_anchor(anchor.anchor_id):
                ...
                logger.info(
                    f"Attestation {attestation_id} anchored: {anchor.anchor_id}"
                )
                return True
    ...
    def get_pending_requests(
        self, attester: Optional[str] = None
    ) -> List[AttestationRequest]:
